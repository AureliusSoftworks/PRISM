...
description: Review PRISM Turnabout proceedings, testimony, exact contradiction targets, objections, rulings, courtroom records, verdicts, and replay, then diagnose provenance and make focused systemic fixes. Use whenever the user invokes /turnabout-review, supplies a Turnabout session or Archive reference, or reports statement, Press, evidence, objection, ruling, floor, voice, completion, or replay failures in a Turnabout court — even without the word "review". Use whodunnit-review for a Whodunnit mystery court.
---
...
1. Read the complete supplied record and resolve exact session and Archive
   ...
   replay evidence is missing when only excerpts are available. Convert every
   developer and session note into a work item before anything else.
...
8. Reproduce the reported role, side, phase, statement/source target, action,
   ruling, and Power state. Add focused regressions; `git diff --check` is
   always cheap. Verify per the core's verification posture, and hand Jared the
   live visual/audio checks when presentation changed.
...
- `Fixes`: systemic changes made, or focused recommendations in review-only
  mode.
- `Verification`: baseline, exact checks run or deliberately not run, lifecycle
  result, and Archive/replay evidence.
