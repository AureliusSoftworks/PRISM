---
name: effort-review
description: Review PRISM experimental/simulated Effort evals, Psychic planning traces, and native-vs-simulated effort behavior, then diagnose provenance and make focused systemic fixes. Use whenever the user invokes /effort-review, pastes an experimental-effort or effort-ladder artifact, asks why simulated thinking helped or failed, reports planning collapse, role-token leakage, private-pass waste, or LOCAL egress risk in the Effort ladder, or wants iterative adjustments to simulated-thinking logic — even without the word "review".
---
...
10. Reproduce the reported session shape with the runbook commands. Add focused regressions for public output and provenance metadata (`psychicDebug`, simulation gate, LOCAL gate). Verify per the core's verification posture; when a run is warranted, narrow API/shared Effort checks first. Keep research-log privacy: aggregates and concise observations only, no full scratchpads.
...
Follow the shared closing retrospective in
`.claude/skills/references/prism-review-core.md` (PRISM root) before the final
handoff, including its evidence gate, review-only boundary, and one-pass limit.
This imports only the closeout step, together with the core's Bead handling and
verification posture; Effort's eval workflow and rules above remain
