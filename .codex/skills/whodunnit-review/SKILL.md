...
description: Review PRISM Whodunnit Case Forge jobs, compiled cases, playable sessions, archives, replays, and portable packages, then diagnose provenance and make focused systemic fixes. Use when the user invokes /whodunnit-review or $whodunnit-review, supplies Case Forge session/case references or error details, cannot create, continue, finish, reopen, export, or import a mystery, or reports case-logic, mansion, evidence, Theory Board, testimony, Court, verdict, voice, asset, or replay failures.
---
...
11. Reproduce the reported session shape with stubs or preserved fixtures first. Add focused regressions for the durable failure cause, public spoiler-safe projection, attempt budget, checkpoint preservation, and playable terminal state. Run narrow shared/API/web checks, typecheck and lint as warranted, `git diff --check`, and player-surface QA for presentation or lifecycle changes.
...
- `Verification`: baseline, exact focused checks, lifecycle result, and whether a saved artifact was reopened.
