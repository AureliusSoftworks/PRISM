---
name: flyting-review
description: Review PRISM Flyting bouts, Hall Records, saved sessions, event streams, crowd sway, Jarl rulings, and replay, then diagnose provenance and make focused systemic fixes. Use when the user invokes /flyting-review or $flyting-review, supplies a Flyting session or Archive reference, asks where a bout went wrong, or reports Bout Forge, consent, claim-targeting, maneuver, Yield, floor, Power, voice, Hall leaning, Jarl verdict, or replay failures.
---
...
1. Read the complete supplied record before judging it. Resolve exact session and Archive references read-only when available. If only screenshots, excerpts, or a Hall Record are available, review them and state which frozen setup, event, provider, audio, crowd-sway, guard, verdict, or replay evidence is missing.
