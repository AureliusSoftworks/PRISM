...
describe("Botcast persistence and isolation", () => {
  ...
  it("keeps physical stage actions out of dialogue while preserving them for ElevenLabs", async () => {
    const db = fixture();
    const provider = recordingProvider(
      [
  ...
  it("asks the opening host to reframe a raw topic organically in persona", async () => {
    ...
    try {
      ...
      assert.match(prompt, /after that hook instead of always leading/u);
  ...
  it("registers Signal background artwork lifecycle and show routes", () => {
    ...
    assert.match(
      serverSource,
      /prompt: withSignalGenerationKeywords\([\s\S]{0,120}promptByKind\[kind\],[\s\S]{0,60}keywords/u,
    );
    assert.match(
      serverSource,
  ...
  it("keeps a repaired guest answer tied to the host's question and the guest's argument", async () => {
    ...
    try {
      ...
      assert.match(
        refusalRepair,
        /My answer follows from|I stand by|To answer/iu,
      );
  ...
  it("cuts an active guest with the host bridge before the closing beat", async () => {
    ...
    try {
      ...
      assert.deepEqual(
        ...
        [
          "The first consequence is that nobody gets to finish—... fine. I'll stop there.",
          "That is where we will leave it. Thank you for listening.",
        ],
      );
      assert.equal(
  ...
  it("honors an earned host sign-off without handing the mic back to the guest", async () => {
    const db = fixture();
    const captures: ProviderMessage[][] = [];
    const provider = recordingProvider(
      [
