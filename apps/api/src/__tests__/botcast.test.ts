...
describe("Botcast persistence and isolation", () => {
  ...
  it("repairs generic Signal recovery with an addressed insult after one focused retry", async () => {
    const db = fixture();
    const captures: ProviderMessage[][] = [];
    db.prepare(
      "UPDATE bots SET name = 'Andy Hominem', powers_json = ? WHERE id = 'guest-1'",
    ...
    try {
      ...
      assert.match(
        ...
        /fresh, tailored personal insult aimed at Mara Vale/iu,
      );
  ...
  it("registers Signal background artwork lifecycle and show routes", () => {
    ...
    assert.match(
      serverSource,
      /const result = await endBotcastEpisodeOnProducerCut\(/u,
    );
    assert.match(
      serverSource,
  ...
  it("creates and renames a stable host-owned show", () => {
    ...
    try {
      ...
      assert.match(
        show.logo.prompt,
        /Design brief, the primary art direction/iu,
      );
  ...
  it("adapts a legacy interruptive host Power into a replay-safe guest cutoff", async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 100; attempt += 1) {
        ...
        if (plan && floorOutcomes.every((outcome) => outcome === "yield")) break;
