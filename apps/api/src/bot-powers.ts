...
function deterministicIdentityMirrorPower(
  ...
  return {
    ...
    observerCue:
      `${subject} knowingly presents as The real <effective target name> after the latest eligible direct address, while borrowing that target's eyes and blink package, complete resting/live mouth package including glyph style and Custom Speech poses, authored Avatar Details Ink, lower glyph, and eligible public Power mechanics and consequences. ${subject} keeps their own persona, behavior, knowledge, convictions, color, material shell, spinner, complete frozen voice and exact Accent Map, bot ID, role, provider, private state, perception, and safety boundaries. Identity Crisis never copies recursively, and private awareness or audience permissions never transfer.`,
...
function deterministicIdentityShapeshiftPower(
  ...
  return {
    ...
    sourceHash: botPowerSourceHashV1(source.name, source.intent),
    selfCue:
      "Each session, take on the complete public form of a random other Library bot (Marketplace if the Library has none): persona, literally double-quoted public name, face, authored ink, saturated color, lower glyph, communication-style chassis, and frame finish. Keep the holder's actual voice identity, provider voice, voice effect, and non-accent shaping; overlay only the target Accent Map region and pronunciation enablement. Stay in that form until short-term amnesia clears continuity, then reshape. Each turn you sincerely know you are the current form. The player is never a target.",
    observerCue:
