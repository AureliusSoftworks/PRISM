...
describe("Zen live presence CSS", () => {
  ...
  it("keeps Zen mouth motion separate from its always-alive metered lamps", () => {
    ...
    assert.match(
      presenceSource,
      /data-talking=\{[\s\S]*?: visualEmissionActive\s+\? "true"/,
    );
    assert.match(
      presenceSource,
    ...
    assert.match(
      presenceSource,
      /<BotAmbientPresenceRig[\s\S]*?isTalking=\{visualEmissionActive\}/,
    );
    assert.match(
      presenceSource,
      /<ZenLiveBotMannequin[\s\S]*?isTalking=\{visualEmissionActive\}/,
    );
    assert.match(
      presenceSource,
  ...
  it("places the bot glyph on the body badge, not inside the action text", () => {
    ...
    assert.match(
      bodyMarkup,
      /<PhosphorPixelSvgGlyph\s+className=\{styles\.zenLiveBotPresenceBotGlyph\}>[\s\S]*?<BotGlyph/,
    );
  ...
  it("quantizes every full-size phosphor glyph on one canonical surface", () => {
    ...
    assert.match(
      pageSource,
      /<PhosphorPixelSvgGlyph\s+className=\{styles\.zenLiveBotPresenceBotGlyph\}>/,
    );
    assert.match(
  ...
  it("routes pre-stream Zen thinking state into the shared face spinner", () => {
    ...
    assert.match(
      thinkingScreenSource,
      /pixelated=\{detailLevel === "full" \|\| detailLevel === "debate"\}/,
    );
    assert.match(
      presenceSource,
