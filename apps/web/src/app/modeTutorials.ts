...
const IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX =
  "An identity-mirroring bot borrows the latest direct bot addresser's public diegetic name, persona, CRT face, authored Avatar Details ink, lower glyph, and the lived consequences of active public Powers. It always keeps its own complete authored voice identity and profile—including accent, pronunciation, speechprint, and client voice effect—plus its saturated color, communication-style chassis, and frame finish. Its screen briefly powers down while the new identity is installed, then reveals the borrowed identity inside that retained shell. The first reveal uses impostor once; later turns inhabit the identity without repeating the label or ever recanting before reset. The offended original briefly corrects a bot that genuinely misaddresses them, even under Credulity, but otherwise advances the conversation. Bot ID, seat, role, private perception, safety, and provider boundaries remain anchored; the player is never copied. The saved handoff, retained voice, and timing replay exactly and reset with the session. When the bot hosts Signal, its authored default identity returns before the closing sign-off so it ends the show as itself.";
...
WHODUNNIT_TUTORIAL_STEP_SOURCE.body = WHODUNNIT_TUTORIAL_STEP_SOURCE.body
  ...
  .replace(
    ...
    "Choose Investigate room to open a search session for free, then move the circular investigation lens across many seeded areas without cost; its glow strengthens nearby without revealing whether a click finds canonical evidence, setting context, a suspicious dead end, or a hidden action token. Each committed frozen area costs one action and resolves once rather than rerolling. Once the current pass is exhausted, the room says All visible areas inspected rather than claiming that every secret has been resolved. Leaving and reopening a room investigation are free.",
  )
  .replace(
  ...
  .replace(
    ...
    "Discovering a room costs one action; revisits, travel between discovered rooms, opening or leaving an interview or investigation, lens movement, Case Kit selection, lead comments, suspect notes, comparisons, and paperclip pins are free. Hotspot inspections, submitted interview questions, and Case Kit applications each cost one action.",
  )
  .replace(
    "On a wide desktop, the investigation opens into a three-part cockpit: the fixed-proportion mansion map anchors the left, a larger current-room stage owns the center, and the Persistent Case Desk becomes a working rail on the right. Narrower windows return the Case Desk beneath the map and stage so every tool remains readable.",
    "On a wide desktop, the investigation opens into a three-part cockpit: the compact mansion map anchors the left, a larger current-room stage owns the center, and the static full-height Case File is a working rail on the right. A thin Gallery-like rack of suspect folders sits beneath the stage; unknown folders stay silhouetted until that suspect’s free Full HD interview. Narrower windows turn the rack into a readable carousel and stack the Case File and desk below the scene.",
  )
  .replace(
    "Newly discovered evidence and committed testimony appear once as durable private notebook references.",
    "Discovered leads, evidence, and testimony remain public records; you choose whether to place two of them side by side on the desk or paperclip one to a revealed suspect as a fallible hypothesis.",
  )
  .replace(
    ...
    "Keyboard, controller, screen-reader, touch, and Reduced Motion paths expose the same broad regions without revealing their outcomes. The desk’s shared Place on desk controls avoid precision dragging, and Escape or a downward pull closes it while clearing only temporary comparisons.",
  )
  .replace(
    ...
    "The Investigator’s Desk is closed by default; pull its handle open to compare two discovered records side by side. Open a revealed suspect folder to see only their known room, interview record, room evidence, explicit confrontations, and linked public leads, then write a plain-text autosaved note or paperclip a discovered lead, evidence, or testimony. Notes and pins are private, fallible player hypotheses—not evidence, theory completion, or hidden truth. The same desk opens on the Theory Board, where you may manually add eligible evidence or testimony to the case.",
  )
      ...
      return index === 0
        ? {
            ...
            body: `${body.replace(
              ...
              "Choose a Coffee Group here to stage its table. Press + to choose one custom Library group or Ungrouped—never individual bots. The table stays linked, so later Library changes update future invitations automatically; each session seats two to four members. ",
