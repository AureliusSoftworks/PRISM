...
WHODUNNIT_TUTORIAL_STEP_SOURCE.body = WHODUNNIT_TUTORIAL_STEP_SOURCE.body
  ...
  .replace(
    "Attach only discovered evidence and exact testimony there, consult your partner freely, complete the culprit, method, motive, opportunity, and record chain, then use Press, Present Evidence, or Pass in the mandatory trial. Three failed court actions spend credibility; PRISM grants one continuance with emergency mansion actions before a second collapse ends the case.",
    "The Theory Board is point-and-click only: select the culprit, accomplice, method, motive, opportunity, evidence, and testimony from the discovered public record. Private desk notes are view-only there; co-counsel consultation is the sole text-entry exception. Filing freezes those selections and the discovered public case record, then visibly gathers the gallery while Debate prepares a real Turnabout on the existing court stage. The sealed Case Bible never enters the public session, bot prompts, replay, or client. Use the Turnabout command deck to Press, Present Evidence, or Pass. Three failed evidence challenges spend credibility; PRISM grants one continuance with emergency mansion actions before a second collapse ends the case. Archive and Resume use the same Turnabout proceeding and gallery record as every other Debate.",
  );
