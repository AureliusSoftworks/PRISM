  ...
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Choose LOCAL or ONLINE as the hard network privacy lane. Private chat is separate: enable it from the new-chat hero when you do not want the conversation or memories saved. Once that conversation begins, a locked Private chat badge remains in the navbar as status, not a switch. Switching to another applet disarms Private chat, so returning Home never starts privately by surprise. Avatar Studio Voice uses clear Accent, Local, and Premium stages: first place the required Accent pin, then shape the Local voice and Feel, then the Premium voice and Feel. One pin controls both engines, so changing from Pia to Rowan—or any other named voice—does not move the bot across regions. Premium and Local share one target-pronunciation path; the pin never changes the spoken language. The Accent stage keeps the full-width map. The world map is a navigator: click a region to zoom in, then click or drag inside the zoomed view to place the pin exactly where you want it, and use the World map button to return to the globe. Zoomed views surface named local variants such as Cockney as explicit chips without guessing identity from location; All accents opens the exact full list. Named regional pins carry their region's whole identity: sound and, where the region has one, its natural turns of phrase in the bot's own replies — pin Cockney and the bot phrases things like a Cockney. Your messages are never restyled, and memory and exports keep exactly what was said. Moving, dropping, or choosing a pin is always silent. To hear a bot, use only the Local, Premium, or Speak buttons beneath that bot in its preview panel. Then set Light, Balanced, or Strong. Voice range filters the named catalog without exposing engine regions.",
        clickLabel: "the LOCAL / ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
...
const DEBATE_COPYCAT_FLOOR_TUTORIAL_SUFFIX =
  "A Copycat advocate repeats the other side’s latest heard public line verbatim—including garbled speech—rather than waiting to be named. If nobody on the other side has spoken yet, they originate one short first floor.";
...
const WHODUNNIT_TUTORIAL_STEP_SOURCE: ModeTutorialStep = {
  ...
  body: "Choose Whodunnit? in Debate format to cast four to eight suspects, a prosecutor partner, and a rival defense bot, then compile a fictional, non-canonical Case Seed. Or tap Surprise me to randomly assign the whole cast at once, then fine-tune suspects or counsel manually if you want. The changing Recipe Seed follows every setup edit; compilation freezes the cast, mansion, one canonical weapon, active room regions, evidence outcomes, proof routes, model lane, Effort, Turbo, Powers, difficulty, and art mode. In play, the compact dark HUD names your mission and keeps Actions, Case file, and Theory reachable while the current room stage, compact mansion map, and persistent Case Notes stay visible together. Co-counsel’s latest reading appears at the lower edge of the stage, while the Case file keeps their consultation composer and public record tools. A seeded fifty-fifty opening either names the weapon’s public descriptor or leaves it to be discovered; its category is a quarter unknown poison, half ordinary object, or a quarter recognizable weapon. The compact mansion map fits beside the Debate stage: its adjacent room boxes preserve their seeded footprints, but every undiscovered room conceals its name, type, icon, occupant, and contents. A visited room reveals its fixed Micro avatar and known details. Selecting a room never travels you; choose Investigate room to travel and update the existing room stage. Inside a suspect room, a wandering Mini avatar invites a focused Full HD interview; click the room outside the interview or press Escape to return to crisp room search. Player questions and bot replies stream in the interview transcript, and configured player and bot voices play through the same PRISM voice path. Talking is free and can be as open-ended or repeated as you like. Return to room search and move the circular investigation lens across many seeded areas; its glow strengthens nearby without revealing whether a click finds canonical evidence, setting context, or a suspicious dead end. Every area remains re-clickable, always repeating its frozen result rather than rerolling it. Newly acquired evidence immediately opens its exhibit detail before joining the private record. Suspect rooms can be searched too. Keys, codes, remotes, and locked containers appear in Case inventory. Drag a usable item onto a locked inventory object, mansion room, or room area; keyboard and controller players can choose Use and then select the same target. A correct pairing may consume the access item and container, unlock a room, or reveal a replacement item. A failed attempt is free and neutral, and harder cases may contain convincing keys with no matching lock. PRISM never labels a recovered object or result as unrelated. Discovering a room costs one action; partner consultation, revisits, travel, interviews, inspecting, access attempts, notebook edits, reference chips, note polishing, and undo are free. Any discovered physical item can go to Forensics for exactly three investigation actions. Its frozen public finding says whether it was used, carries contextual traces, or has no trace matching the known method; it never exposes another secret. Type @ in the interview composer or partner composer to mention discovered evidence, committed testimony, a colored suspect name, or the victim. An evidence mention formally confronts a suspect; merely naming an item is ordinary prose. Newly discovered evidence and committed testimony appear once as durable private notebook references. Keyboard, controller, screen-reader, and Reduced Motion paths expose the same broad regions without revealing their outcomes. Case Notes stay inline beneath the compact map; they are private, fallible, autosaved, Markdown-friendly, and never shared through Seeds or treated as evidence. Only populated authored blocks are added; PRISM safely reconciles automatic one-window reference revisions before reporting a real conflict, and safely polishes authored notes after editing; Review page polish still presents a meaning-preserving diff before a broader rewrite is applied. Attach only discovered evidence and exact testimony to the Theory Board, complete its culprit, method, motive, opportunity, and record chain, then use Press, Present Evidence, or Pass in the mandatory trial. Three failed court actions spend credibility; PRISM grants one continuance with emergency mansion actions before a second collapse ends the case. Archive shows the deterministic Smoking Gun, Strong Case, Lucky Break, or Incorrect grade first. The full timeline, accomplice, unseen clues, and unused proof routes stay behind Reveal spoilers, while Copy Case Seed recreates the frozen case logic with a blank notebook and fresh visuals.",
...
const WHODUNNIT_TUTORIAL_STEP: ModeTutorialStep = {
  ...
  body: WHODUNNIT_TUTORIAL_STEP_SOURCE.body
    .replace(
      "Inside a suspect room, a wandering Mini avatar invites a focused Full HD interview; click the room outside the interview or press Escape to return to crisp room search. Player questions and bot replies stream in the interview transcript, and configured player and bot voices play through the same PRISM voice path. Talking is free and can be as open-ended or repeated as you like.",
      "Inside a suspect room, click the wandering Mini avatar to focus a Full HD interview over softened room art; click the room outside the interview or press Escape to return to crisp searchable art. Suggested questions only fill the composer, so you decide when to ask. The player transcript streams before the bot thinks, streams its reply, and both configured voice paths play in the same room. Talking is free, concise, and can be repeated as you like.",
    )
    .replace(
      "The Case file is one compact tool surface: switch between Counsel, Access, Evidence, and Testimony instead of opening a stack of dashboard cards.",
      "The Case file is one compact tool surface: switch between Counsel, Leads, Access, Evidence, and Testimony instead of opening a stack of dashboard cards. PRISM automatically adds and updates several lead threads from discovered public facts, announces each change, and never pretends every thread will reconcile; some can stall or remain unresolved.",
    )
    .replace(
      "Type @ in the interview composer or partner composer to mention discovered evidence, committed testimony, a colored suspect name, or the victim.",
      "Type @ in the interview composer or partner composer to mention discovered evidence, committed testimony, a colored suspect name, or the victim. The partner composer can also @ a lead at its current revision, so co-counsel reasons from the exact public thread you are viewing.",
    )
    .replace(
      "Your dark Investigator’s Notebook drawer is available throughout the case, private, fallible, autosaved, Markdown-friendly, and never shared through Seeds or treated as evidence.",
      "The notebook icon opens persistent side chrome to those automatically maintained Leads, lets you attach your own fallible annotation to each one, and keeps separate freeform Notes available throughout the case while the stage narrows slightly. It is private, autosaved, Markdown-friendly, and never shared through Seeds or treated as evidence.",
    ),
};
  ...
  debate: {
    ...
    steps: [...BASE_MODE_TUTORIALS.debate.steps.map((step) => {
      ...
      if (step.heading === "Enter the Debate Studio") {
        return {
          ...step,
          body: `${DEBATE_PRE_SESSION_ALIGNMENT_TUTORIAL_PREFIX} ${body}`,
        };
      }
        ...
        steps: tutorial.steps
          ...
          .map((step, index) => {
            ...
            const compactAvatarSpeechStep =
              mode === "avatar" &&
              currentStep.heading === "Tune it with live controls"
                ? {
                    ...currentStep,
                    body: currentStep.body.replace(
                      "Micro keeps its face and Ink through 29px.",
                      "Mini keeps the full live mouth-shape stream. Micro deliberately swaps each open mouth pose to a literal 0, keeps its resting mouth for closures, then returns to rest. Micro keeps its face and Ink through 29px.",
                    ).replace(
