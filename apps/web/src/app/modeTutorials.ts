...
const BASE_MODE_TUTORIALS: Record<TutorialMode, ModeTutorial> = {
  ...
  zen: {
    ...
    steps: [
      ...
      {
        heading: "Give the group its own mark",
        body: "Each saved group carries a small triangle glyph with its shared color field. Reroll glyph changes only that visual mark, so the group’s members, name, room, and order stay exactly as they are. Right-click a bot card inside the group and choose Promote to leader to give that bot the crown; choosing another bot simply reassigns it. A leader’s chassis carries the bot-colored Effort shape, with one point for each group they lead. The group mark and leader follow the group through exports and backups.",
        clickLabel: "Reroll glyph in the saved group header",
        targetSelector: '[data-tutorial-target="chat-group-glyph-reroll"]',
      },
      {
      ...
      },
      {
        ...
        body: "On desktop Zen, the anchored spectrum string navigates large bot libraries in two directions. Drag sideways to choose a hue. Pull upward to narrow that hue toward the intimate two-row neighborhood; pull downward to broaden it until the full rainbow returns. The room's gradient mirrors the visible directory with two to five color nodes—five is the maximum at five rows and beyond—and the root restores the full five-color PRISM atmosphere. The bot grid reshapes live, and releasing the string lets it wobble home without changing the hue or breadth you chose. All clears both axes. While search is open, the string dims and search scans the entire active group; clearing search restores your exact prior place. Keyboard users can focus Hue and Breadth separately: Left/Right changes hue, Up/Down changes one row, Home returns to the remembered-hue root, End opens two rows, and Escape returns to All.",
        clickLabel: "the spectrum string",
        targetSelector: '[data-tutorial-target="zen-hue-string"]',
      },
    ],
  },
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Choose LOCAL or ONLINE as the hard network privacy lane. Private chat is separate: enable it from the new-chat hero when you do not want the conversation or memories saved. Once that conversation begins, a locked Private chat badge remains in the navbar as status, not a switch. Switching to another applet disarms Private chat, so returning Home never starts privately by surprise. Avatar Studio Voice uses clear Accent, Local, and Premium stages: first place the required Accent pin, then shape the Local voice and Feel, then the Premium voice and Feel. One pin controls both engines, so changing from Pia to Rowan—or any other named voice—does not move the bot across regions. Premium may use a more specific named accent as accented English, while Local uses the nearest qualified Speechprint approximation; the pin never changes the spoken language. Zen player voice settings keep the full-width map in Accent. Click or drag anywhere in the world: the pin stays exactly where you leave it while Prism approximates the nearest broadly regional pronunciation and rhythm—including New York and Southern U.S. Nearby choices expose the closest named regions when several share a small map area; All accents opens the exact full list. Then set Light, Balanced, or Strong. Voice range filters the named catalog without exposing engine regions. Original and With accent compare the same selected voice and engine; dialogue text, memory, and exports stay unchanged.",
        clickLabel: "the LOCAL / ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
  ...
  debate: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Describe the idea in ordinary words and choose Build the debate. Prism makes one balanced motion, names both sides, and writes the briefs you should not have to author. Try another version gives you a fresh framing; Refine motion reveals the alternate motions and direct field editing only when you want them. Prompt Center prompts insert as ordinary editable text, while wildcard rolls stay as chips until you Build or Refract—the surprise fills then, not while you type. A `{VAR}` inside a Prompt Center body means whatever you typed after the prompt. You may repeat `{VAR}` in that body; there is only one shared capture and no A/B/C letter links. The floating Prism remains available throughout setup and understands the bounded, unsaved workbench draft. Wield Prism into a glowing setup field for a contextual editable candidate shaped by the current room, role, cast, motion, and evidence. A shimmering field is locked while Prism works. Wield a different registered input to queue it once; Prism processes unique inputs in click order, while repeat clicks on the active or already-queued input do nothing. After the final draft settles, Space rerolls and clicking away, Enter, or Tab accepts. Escape restores the active field and clears the remaining queue. Your Prism shortcut opens the assistant menu at the orb instead. The idea dice remains available for a fast local seed.",
      ...
      },
      {
        ...
        body: "Spectators and human Judges can open the four-seat Jury camera manually once leanings, deliberation, or ballots begin (the control stays visible but disabled until then), and Auto also enters the chamber for those same beats before returning to the forum for advocate aftermath reactions. While the Jury camera is up, the public gallery strip hides so the chamber owns the stage. The same CC control that toggles Forum captions also shows or hides spoken Jury chamber subtitles. The four jurors follow the live case and trade short reactions between public-floor turns. Four private leanings lead into routed discussion and four final juror ballots. In bot-moderated sessions, the Moderator then records a distinct fifth and final ballot on the Moderator camera, breaking a 2–2 split while still voting after a 3–1 or 4–0 juror result. For a human Judge, no vote is invented on your behalf: the four-juror chamber remains advisory and your own final ruling occupies that last authority beat. Participants never mount this chamber; their camera stays on the public proceeding while only the aggregate resolves and individual ballots remain private.",
...
const IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX =
  "An identity-mirroring bot borrows the latest direct bot addresser's public diegetic identity: name, persona, CRT face, authored Avatar Details ink, resolved voice identity, lower glyph, and the lived consequences of active public Powers. It keeps its own saturated color, client-side voice effect, communication-style chassis, and frame finish. Its screen briefly powers down while the new identity is installed, then reveals the borrowed identity inside that retained shell. Bot ID, seat, role, private perception, safety, and provider boundaries remain anchored; the player is never copied. The saved handoff and its timing replay exactly and reset with the session. When the bot hosts Signal, its authored default identity returns before the closing sign-off so it ends the show as itself.";
...
const PRISM_COMPANION_VIEWS_TUTORIAL_SUFFIX =
  "The floating Prism panel can switch between Synthesis, Chat, and Notes. Synthesis shows the live image job when one is running; otherwise its empty + accepts a prompt and opens that prompt in Images without generating until you choose Synthesize. Chat keeps the same saved or Private Prism conversation and focused-chat handoff. Notes reuses the session-note composer for encrypted personal notes you can create, reopen, edit, and delete; personal Notes stay unavailable while Prism Chat is Private. On the default bot overview, Prism stays in its Home mark and follows live layout or screen changes. Opening Settings, Avatar Studio, Images, Memories, Usage, Prompt Center, Bots, or another top-bar panel visually submerges the passive orb while keeping Wield and contextual field population available above the panel. Closing the panel restores Prism on the underlying surface. On ordinary surfaces, your Prism shortcut opens this menu at the orb's current location. With the menu closed, Wield Prism onto an eligible text field for a contextual editable draft; its moving spectrum stays visible and the field stays locked until the text appears. Keep Command held on macOS or Control held on Windows and Linux, then click distinct eligible inputs to queue each once in click order; Prism fills them consecutively, while repeat clicks on the active or already-queued input do nothing. The same safety boundaries remain: passwords, credentials, disabled fields, destructive confirmations, live production, and replay remain untouched. While a Prism menu is open, the Wield modifier leaves the assistant anchored; close the menu before you Wield it.";
...
function currentSignalRefractTutorialBody(body: string, index: number): string {
  const current = body
    ...
    .replace(
      ...
      "Refract replaces the old Topic and Private comments dice: hold Option to Wield Prism, then click either glowing registered field, or drag the Prism orb onto it. The active field locks while its spectrum moves. Option-click a different registered input to queue it once; Prism processes unique inputs in click order, and repeat clicks on the active or already-queued input do nothing. After the final draft settles, Space rerolls and clicking away, Enter, or Tab keeps it. Escape restores the active field and clears the remaining queue. Your Summon / Wield Prism shortcut—Command + Option by default—opens the assistant menu at the orb instead. Shortcuts can be changed in Settings → Shortcuts.",
    );
  if (index === 0) {
...
function currentDebateJuryTutorialBody(body: string): string {
  return body
    ...
    .replace(
      ...
      "If Jury is enabled, PRISM automatically visits the chamber for leanings, audible deliberation, and juror ballots, cuts to the Moderator for the final ballot when the Moderator is a bot, returns for the split, then restores the forum before advocate aftermath reactions. Spectators and Judges also get a manual Jury camera once that chamber window opens, as an escape hatch if Auto leaves them on the forum; it stays disabled until then. Participant and Spectator sessions retain manual public-floor cameras outside that Jury passage.",
    )
    .replace(
...
function currentSimulatedEffortTutorialBody(body: string): string {
  return currentChatZenPresentationTutorialBody(body)
    ...
    .replace(
      "Hold Control for a moment to reveal a small shortcut toast for that Control-root cluster; Wield Prism stays legend-free. These shortcuts can be changed in Settings → Shortcuts.",
      "Hold Control for a moment to reveal a small shortcut toast for the PRISM command layer; Wield Prism stays legend-free. Option+Command summons Prism, while holding Option alone still Wields it. These shortcuts can be changed in Settings → Shortcuts.",
    )
