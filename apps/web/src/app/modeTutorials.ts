  ...
  botcast: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Signal captures the actual voice audio, camera choices, production events, and frozen cast appearance while the episode is live; it never asks an AI to recreate the episode. Selecting an archived episode without a finished video starts or resumes its deterministic Studio render without opening anything, and the episode card shows progress. Select it again after Video ready to open the browser-playable video. The video carries the saved set, cast, camera cut, captions, departures, voice changes, and crosstalk. You can play, pause, scrub, download the video or transcript, Retry when a frozen voice is unavailable, and Delete Recording without deleting the episode transcript. Closing PRISM leaves the durable job queued until a capable client selects the episode again. There are no post-episode camera controls and no second AI turn. At the end card or in the saved episode, Copy for Signal Review puts the complete conversation plus its private cues, per-turn model routing, delivery notes, segment changes, camera decisions, and outcome on your clipboard for a focused review. Delete episode sits beside it only after the broadcast is complete and remains the separate destructive action.",
