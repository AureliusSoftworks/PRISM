...
const MODEL_ROUTING_VISIBILITY_TUTORIAL_SUFFIX = (
  ...
  .replace(
    "Every visible, runnable text model in the selected privacy lane is eligible for contextual Auto. LOCAL evaluates only local Ollama models; ONLINE evaluates only configured OpenAI and Anthropic models.",
    "Every enabled, runnable text model in the selected privacy lane is eligible for contextual Auto. LOCAL evaluates only local Ollama models; ONLINE evaluates only configured OpenAI and Anthropic models. In Settings → Models, Show in picker controls only whether an enabled model appears in manual model pickers; it never removes that model from Auto routing.",
  )
  .replace(
