import { DEBATE_EVIDENCE_SOURCE_MAX_COUNT } from "@localai/shared";
    ...
    steps: [
      {
        ...
        body: "A Debate is a saved exchange between two advocates. Basic setup is the welcoming default: give Prism a topic, choose the debaters, and keep sources optional. New Duel clears the active workbench without touching archived proceedings.",
        clickLabel: "New Duel",
        targetSelector: '[data-tutorial-target="debate-new"]',
      },
      {
        heading: "Choose Basic or Advanced",
        body: "Basic asks only for the debate idea and two debaters. Prism writes the balanced motion and both private side briefs, uses a Plainspoken Forum, holds the center Judge seat silently until you act, leaves the final call to you, and keeps the Jury off. Advanced preserves the full studio: presets, five formality levels, Forum or Turnabout, player roles, editable briefs, and the optional Jury. Daytime Showdown keeps its confrontation-show energy when you take the Judge role; Custom records the role change without turning Free-for-all into a polite panel. You can switch at any time; returning to Basic applies its simpler room defaults without deleting your topic, cast, or sources.",
      ...
      },
      {
        ...
        body: "In Basic, describe the idea in ordinary words and choose Build the debate. Prism makes one balanced motion, names both sides, and writes the briefs you should not have to author; Try another version gives you a fresh framing. Advanced restores three motion choices and direct editing of every field. The territory dice remains available for a fresh seed.",
        clickLabel: "Build the debate",
        targetSelector: '[data-tutorial-target="debate-synthesize"]',
      },
      {
        heading: "Cast the room",
        body: "Basic asks you to choose the two advocates while Prism takes the center Judge / Moderator seat but stays publicly silent and inactive until you act. Your gavel, question, pause, resume, and final ruling use that same avatar and player voice; Prism never invents a Judge action between them. Surprise me can fill both sides. Advanced adds Participant and Spectator roles, a Library moderator, Random actors, and the optional five-seat Jury. Only duplicate seating is blocked: Powers never make a bot ineligible for a role.",
        clickLabel: "Debaters",
        targetSelector: '[data-tutorial-target="debate-cast"]',
      },
      {
        heading: "Secure advocacy consent",
        body: "Choose Make sure they’re willing after the question and debaters are set. This private LLM check protects the same consent contract in both modes: a genuine boundary can decline and cannot be overridden; a willing Devil’s Advocate keeps their identity and receives visible framing. Changing the motion, cast, format, or formality clears the old check.",
        clickLabel: "Make sure they’re willing",
        targetSelector: '[data-tutorial-target="debate-consent"]',
      },
      {
        heading: "Freeze one shared record",
        body: `Sources are optional. In AUTO or ONLINE, Find sources for me runs a real-source Brave search from the motion. Those results lock into the packet, and later searches add distinct sources without replacing them, up to ${DEBATE_EVIDENCE_SOURCE_MAX_COUNT}; remove any source you do not want. Prism never fabricates sources. Advanced also exposes manual Brave search. LOCAL blocks research before network access, and Debate never reads or writes relationship memory.`,
        clickLabel: "Sources",
        targetSelector: '[data-tutorial-target="debate-evidence"]',
      },
      {
        heading: "Read the launch circuit",
        body: "The ready check stays visible beside every Studio tool. It shows whether the question, debaters, and consent are ready while keeping optional sources out of your way.",
        clickLabel: "the ready check",
        targetSelector: '[data-tutorial-target="debate-readiness"]',
      },
      {
        heading: "Open the proceeding",
        body: "Choose LOCAL, AUTO, or ONLINE and a primary Debate model in the navbar before setup work. AUTO quietly retries a failed or malformed generation through the fallback chain saved in Settings. Start freezes that ordered chain and the format version with the cast, sampled Jury, motion, consent, evidence, and resolved Powers; every generated statement and ballot records the model that actually handled it. In a human-Judge session, bot-controlled floors advance around an inactive center seat: no Judge-originated public event appears until you act. If the whole chain fails, the proceeding pauses on the exact action and offers Retry or Skip without dialogue; Jury ballots are Retry-only. LOCAL remains a hard offline guarantee. Live Pause first calls the room to order with the moderator’s gavel and announcement, then suspends the proceeding and restores the entire navbar. Resume has the moderator or Judge call the proceeding back to order with a saved gavel action, preserves the exact next juror, discussion turn, ballot, or interrupted line, and locks the live-session controls again. Pause and Resume remain live procedural presentation only; neither housekeeping beat enters the readable proceedings or copied transcript. A human Judge waits through the same eight-second gavel cooldown before the context gavel or Pause becomes available again; bot-moderated roles may Pause again immediately. End early skips the remaining rounds: a Jury holds only three discussion turns and is told not to penalize unheard rounds. When formal Jury deliberation arrives, a timed choice appears over the current camera. Auto is the default and begins after the Debate-settings countdown; Participate manually enters the chamber, while Skip moves directly to final ballots. Skipping remains available after discussion starts and preserves all five final ballots and the full five-ballot Jury result. Judge and Participant input otherwise waits indefinitely until you act or pass.",
        clickLabel: "Start Forum or Start Turnabout",
        targetSelector: '[data-tutorial-target="debate-start"]',
      },
      {
        heading: "Use the Judge’s gavel",
        body: "When you are the Judge, Prism’s center seat stays publicly silent and inactive until you act. PRISM never invents a phase announcement, fallback challenge, overtime correction, interruption ruling, ballot, or gavel strike for you. An advocate cutoff is different: no gavel predicts it. The interrupter literally shouts “Objection!” and states the challenge first. A timed Sustained / Overruled choice then appears. Sustained leaves the cutoff in place; Overruled returns the floor for a concise answer and continuation. If time expires, Overruled keeps the Debate moving. Gavel stays available throughout the live proceeding, and Space swings it without reaching for the button. Every actual slam briefly forces the Judge / Moderator view and locks camera controls through the swing; your selected camera mode is preserved, so Auto immediately resumes directing after the strike. At saved procedural moments, a ceremonial cue waits for your strike without authoring one for you. If that cue expires, the interface stays clear while Auto silently cuts to one advocate and then the moderator before the proceeding carries on. During an advocate’s overtime, the first strike stops the speaker and opens a two-second procedural burst. One strike earns a measured call-time line, two or three make Prism firmer, and four or more make the Judge sound aggravated. The whole burst remains one call-time action: it never opens choices, creates another message or ruling, or changes the resumed schedule. Overtime calls bypass any settling intervention cooldown without starting, extending, or erasing it. Outside overtime, the gavel calls the room to order and opens the Judge’s choice deck. End Debate closes the floor and moves directly to your final ruling. Three answer-seeking prompts route the advocate best positioned to respond once, then resume the exact interrupted schedule. Write my own reveals the standard composer at the bottom: type there, or use its dice to draft editable Judge prose before sending. A custom procedural command, ruling, declaration, or call for order is entered without fabricating an advocate rebuttal; a direct question still receives one concise answer. A deliberately quiet nevermind choice resumes the interrupted schedule without adding Judge prose or a public Pass event. Final rulings stay one-tap and still require a side. For two seconds after an ordinary intervention strike, hammer Space as much as you like: those extra smashes are pure showmanship. The eight-second server cooldown prevents a second deliberation, not a procedural overtime call.",
        clickLabel: "Gavel",
        targetSelector: '[data-tutorial-target="debate-judge-gavel"]',
      },
      {
        heading: "Read the living case",
        body: "Both formats stage each advocate's actual animated bot behind an authored side podium, with the moderator elevated between them when the frozen cast can perceive that body. The motion stays titled across the top of the chamber while perceptible words appear as synchronized broadcast captions along the bottom. Public prose arrives with the live voice; inaccessible speech never enters captions, voice, the shared case board, or listener-facing ballot reasons. On a sparse replay-stable roll, an advocate or visible juror whose own saved Persona genuinely finds an audible contribution contrary to expectation or newly explanatory may give one short in-character vocal reaction. It is atmosphere, not a new argument, vote, role change, relationship-memory read, or hidden Power reveal. A neutral floor marker keeps the proceeding legible without exposing hidden words. Each visible podium carries its bot's glyph; the bot that owns the current turn glows even when the turn is silent, so the cue follows floor ownership rather than speech or prose while stable bot ID remains canonical. Their frozen faces, ink, frame finishes, visibility, thinking, listening, and speaking states remain live throughout the proceeding. Judge choices take over the caption position at the moment of decision; Participant and Turnabout actions still rise in a full-width command deck. Forum keeps the scoreless case board and Participant interjections; only the heard fragment remains public before the moderator rules. Turnabout replaces that sequence with a public statement record: Press asks for clarification; Object opens the frozen evidence vault; Present Evidence sends one statement-and-evidence pair for grounded validation and an immediate ruling. Sustained contradictions create explicit reversals without inventing evidence. With Jury off, reduced-detail spectator bots face neighboring seats and trade quiet ellipsis chatter without gaining the floor or a vote; a dry gallery murmur remains a separate audio layer. With Jury on, that strip becomes the frozen public roster.",
        clickLabel: "the living case board",
        targetSelector: '[data-tutorial-target="debate-case-board"]',
      },
      {
        heading: "Enter the Jury chamber",
        body: "Spectators can open the five-seat Jury camera manually at any point. The jurors follow the live case and trade short reactions between public-floor turns. An ellipsis beside a juror means a thought is waiting; enter the Jury camera before the next thought and that juror will deliver it, while another camera lets them resolve immediately without holding up the proceeding. PRISM matches the Light or Dark table, seats bot faces and frames around its transparent foreground, and carries active-speaker color, gaze, thinking, listening, speech, voice, and applicable visual Powers into the room. Five private leanings lead into five short routed discussion turns and five final ballots. Each audible juror reads the same final reason saved in the Jury record while their anonymous mark slides into a pile at the table’s center; a canonically silent juror still casts without gaining a voice. The foreperson then reads the split. Judges / Moderators and Participants never mount this chamber; their camera stays on the public proceeding while five anonymous marks resolve into the aggregate. Auto and every Forum camera stay out of the chamber unless an eligible Spectator chooses Participate or Jury.",
        clickLabel: "the Jury chamber",
        targetSelector: '[data-tutorial-target="debate-jury-chamber"]',
      },
      {
        heading: "Frame the floor",
        body: "Auto is the quiet default camera: it cuts instantly to Left for the For advocate, Moderator for the moderator, Right for the Against advocate, and Wide whenever no bot owns the public floor. When you take the Judge / Moderator seat, Auto is forced for the entire session: manual camera choices and Participate entry to the Jury are unavailable, and every forced gavel shot returns directly to Auto. Participant and Spectator sessions retain manual cameras; Spectators can choose Jury manually at any point or choose Participate in the timed deliberation prompt. Auto never enters the Jury chamber on its own. In Participant and Spectator sessions, procedural gavel cues direct Auto to Moderator: one strike calls attention at every phase change, while two restore order for moderator rulings and verdicts, with the active moderator’s color carried through the instrument. Advocate objections carry no predictive gavel cue; the objection is heard before any bot moderator responds. A human-Judge session has no automatic moderator ceremony or camera claim; explicit Judge actions alone activate the center seat and gavel. Any actual gavel slam briefly forces Moderator and disables camera controls through the swing without replacing the selected mode; Auto resumes as soon as the forced shot ends. A canonically silent bot moderator can use that visible signal without speech. Forum and Turnabout keep the procedural rhythm for bot-moderated roles; Turnabout keeps an extra strike for a public revelation. The gavel is visible only in Moderator view. Choose a manual view to hold the shot outside forced strikes when your role allows it. Camera choice changes presentation only—it never changes the saved transcript, case board, ballots, or speaking order.",
        clickLabel: "a Debate camera",
        targetSelector: '[data-tutorial-target="debate-camera"]',
      },
      {
        heading: "Follow and keep the record",
        body: "Proceedings render safe Markdown and source chips in the chamber's tonal transcript rail. It follows every growing live turn until you deliberately scroll back; choose Live to return to the newest phrase. Juror thoughts and deliberation stay out of Proceedings. After the verdict, Judges and Spectators get a separate timestamped Jury record with its own copy action, including between-turn thoughts they did not enter the chamber to hear. That Jury transcript remains directly copyable from its eligible Proceeding archive entry after you return to the Studio. With Jury off, a Judge's ruling is final and completes without moderator or advocate bot ballots; Participant or Spectator Duels still use the traditional three-cast majority. With Jury on, the majority binds Spectators and Participants but advises a human Judge. A Jury verdict is followed by both advocates’ public reactions; the bot Moderator then closes Spectator and Participant sessions, while the human Judge’s ruling closes Judge sessions. Judge and Spectator records keep named deliberation and ballots; Participant API responses, transcript copies, archives, and replay-facing event data retain only the aggregate split and verdict. Copy verbose transcript creates one review-ready role-safe record with frozen setup, runtime snapshots, evidence, event metadata, interruptions, moderator rulings, case-board state, and permitted public ballot reasons.",
    ...
    steps: [
      {
        ...
        body: "Choose one source for new work: a creative spark or pages you already wrote. Bringing existing material replaces the spark controls so Slate never blends the two; optional {wildcards} remain only for spark-led work. Confirm the working title yourself, or let Slate suggest one and keep the final decision. Mirror setup is never required before you can write.",
