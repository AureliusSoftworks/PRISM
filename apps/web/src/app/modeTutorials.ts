...
WHODUNNIT_TUTORIAL_STEP.body = `${WHODUNNIT_TUTORIAL_STEP.body} Every dialogue caption types out at a deliberate courtroom-game pace. The current line's Babble or spoken performance follows that reveal; click, Enter, or Space once to complete and silence it immediately, then use the next gesture to advance. Room observations are attributed to the player character, use Babble whenever written accompaniment is on, and never auto-close. Only observations that produce a durable public case consequence—an admitted record, Case Kit recovery, or unlocked lead—enter the Case File's Observation Log with their room and player attribution. Frozen keys, codes, remotes, and other access objects also enter the Case Kit with an immediate acquisition cue. Settings → Debate lets other non-spoken dialogue follow Off, Babble, or Bottish while its text appears; spoken character TTS is unchanged.`
  .replace(
    "On a suspect’s first room entrance only, dismissing the anonymous Casekeeper tableau may spend up to two seconds on one optional Auto-routed cadence choice in the case’s frozen LOCAL or ONLINE privacy lane, then prepare that exact line locally; timeout, invalid output, or unavailable frozen voice keeps the canonical text and verified clip. Archive and replay reuse the persisted result without another model or voice call.",
