  ...
  zen: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "The composer at the bottom starts a fresh user-first one-on-one with this bot; it never silently appends to an older conversation. Enter sends, Shift+Enter adds a line, and a failed handoff leaves the message available in the destination composer.",
      ...
      },
      {
        ...
        body: "On desktop Zen, the Hue Cable navigates large bot libraries in two directions. Drag sideways past the small pull threshold to choose a hue. Pull upward to narrow that hue toward the intimate one-row card gallery; pull downward to broaden it until the full rainbow returns. From any state, a full vertical yank spans the complete available depth ladder, whether your library holds hundreds or thousands of bots. The room's gradient mirrors the visible directory with one to five color nodes—five is the maximum at five rows and beyond—and the root restores the full five-color PRISM atmosphere. The bot grid reshapes directly, and releasing the cable snaps it back without changing the hue or breadth you chose. All clears both axes. While search is open, the cable dims and search scans the entire active group; clearing search restores your exact prior place. Keyboard users can focus Hue and Breadth separately: Left/Right changes hue, Up/Down changes one row, Home returns to the remembered-hue root, End opens one row, and Escape returns to All.",
  ...
  debate: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Describe the idea in ordinary words and choose Build the debate. Prism makes one balanced motion, names both sides, and writes the briefs you should not have to author. Try another version gives you a fresh framing; Refine motion reveals the alternate motions and direct field editing only when you want them. Prompt Center prompts insert as ordinary editable text, while wildcard rolls stay as chips until you Build or Refract—the surprise fills then, not while you type. A `{VAR}` inside a Prompt Center body means whatever you typed after the prompt. You may repeat `{VAR}` in that body; there is only one shared capture and no A/B/C letter links. The floating Prism remains available throughout setup and understands the bounded, unsaved workbench draft. Wield Prism into a glowing setup field for a contextual editable candidate shaped by the current room, role, cast, motion, and evidence. The shimmering field stays read-only while Prism works, but the rest of the Studio remains fully usable: click and type elsewhere without interrupting it. Click the active rainbow sheen again to cancel; otherwise it continues until it settles or you leave the Studio. Wield a different registered input to queue it once; Prism processes unique inputs in click order. After the final draft settles, Space rerolls and clicking away, Enter, or Tab accepts. Escape restores a settled draft and clears the remaining queue. Your Prism shortcut opens the assistant menu at the orb instead. The idea dice remains available for a fast local seed.",
...
function currentSignalRefractTutorialBody(body: string, index: number): string {
  const current = body
    ...
    .replace(
      ...
      "Refract replaces the old Topic and Private comments dice: hold Option to Wield Prism, then click either glowing registered field, or drag the Prism orb onto it. The shimmering field stays read-only while Prism works, but Signal remains fully usable: click and type elsewhere without interrupting it. Click the active rainbow sheen again to cancel; otherwise it continues until it settles or you leave Signal. Option-click a different registered input to queue it once; Prism processes unique inputs in click order. After the final draft settles, Space rerolls and clicking away, Enter, or Tab keeps it. Escape restores a settled draft and clears the remaining queue. Refract follows the global LOCAL/ONLINE, Model, and Effort controls; it never borrows the Background model. Your Summon / Wield Prism shortcut—Command + Option by default—opens the assistant menu at the orb instead. Shortcuts can be changed in Settings → Shortcuts.",
    );
  if (index === 0) {
...
function currentChatZenPresentationTutorialBody(body: string): string {
  return body
    ...
    .replace(
      ...
      "Assistant text uses its own visual clock in both views: immersive Zen keeps the normal canvas cadence, transcript Chat reveals substantially faster, and spoken audio continues naturally at its own pace.",
    );
}
