...
function currentDebateJuryTutorialBody(body: string): string {
  return body
    ...
    .replace(
      ...
      "If Jury is enabled, PRISM automatically visits the chamber for leanings, sealed deliberation, ballots, and the split, then returns to the forum before advocate aftermath reactions. Spectators and Judges also get a manual Jury camera once that chamber window opens, as an escape hatch if Auto leaves them on the forum; it stays disabled until then. Participant and Spectator sessions retain manual public-floor cameras outside that Jury passage.",
    )
    .replace(
