...
const IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX =
  "An Identity Crisis bot knowingly wears the latest eligible direct addresser’s effective public presentation: exact public name, eyes and blink package, complete resting and speaking mouth package including glyph style and Custom Speech poses, authored Avatar Details Ink, and lower glyph. It also copies the target’s eligible public Powers and consequences. The holder remains fully itself in persona and behavior and keeps its color, material shell, complete frozen voice and Accent Map, chassis/frame/spinner, bot ID, seat, role, privacy, safety, and mechanics. Identity Crisis never copies recursively, and private awareness or audience permissions never transfer. The screen briefly powers down while a genuinely new target is installed, then reveals it; saved timing replays exactly and resets with the session. Identity Mirror presentation has precedence over Shapeshifter. Coffee and Signal target bots; Whodunnit V2 can also target its player-controlled Prosecutor.";
const IDENTITY_SHAPESHIFT_POWER_TUTORIAL =
  "A Shapeshifter sincerely becomes a different Library bot's public persona and complete visual form—public name, CRT face and thinking spinner, authored Avatar Details Ink, saturated color, lower glyph, communication-style chassis, and frame finish—for the session. The holder keeps its actual voice identity, provider voice, voice effect, and every non-accent shaping field; only the target Accent Map region and pronunciation on/off state overlay the voice. A target with pronunciation off disables the transformed accent. Short-term amnesia reshuffles; Marketplace is the fallback when no other Library bots exist. Its screen briefly powers down only for a genuinely new form, installs the identity while dark, then reveals it; reloads and ordinary rerenders do not restart the change. Mechanical seat, Powers, and hard speech rules stay with the holder, and the player is never a target. Identity Crisis still wins presentation when both are active.";
...
const FLYTING_TUTORIAL_STEP: ModeTutorialStep = {
  ...
  body: "Choose Flyting to open a distinct contest of answering wit—not a Viking skin over Forum or Cypher. Summon chooses whether you coach one bot, host the contest, or watch. Cast seats two flyters, an optional bot Host, and four Hall members. Bout Forge creates editable fictional stakes, epithets, and three public Legend facets per flyter without private relationship memory or live research; both flyters must consent before Start or Save. Four exchanges alternate Boast, a targeted Doubt, Expose, Belittle, or Outdo challenge, and a Stand, Own, Turn, or Return rejoinder. Your line always begins blank; write it or use Wield PRISM for one editable draft. A rejoinder may Yield, which permanently records the charge as unanswered. There is no timer, fixed meter, or rhyme requirement. The Hall Record preserves every exact claim, answer, Power-shaped public delivery, and acclamation. Four Hall votes advise a human Host, who must crown one winner; a bot Host casts the fifth deciding vote. Flyting uses brief procedural ritual cues and bot voices, never continuous music. LOCAL stays fully offline, and Archive reopens the same frozen bout and record.",
...
WHODUNNIT_TUTORIAL_STEP.body = `${WHODUNNIT_TUTORIAL_STEP.body} Settings → Debate lets written Casekeeper observations and other non-spoken dialogue follow Off, Babble, or Bottish while their text appears. Bottish is the default; anonymous Casekeeper speech keeps its authored Babble carrier, and spoken character TTS is unchanged.`
  ...
  .replace(
    ...
    "Identity Crisis knowingly wears the latest eligible direct addresser’s effective public name, exact eyes and blink package, complete resting and speaking mouth package including glyph style and Custom Speech poses, Avatar Details Ink, and lower glyph, and copies that target’s eligible public Powers and consequences. The holder stays itself in persona and behavior and keeps its color, material shell, complete frozen voice and Accent Map, chassis/frame/spinner, and every private or mechanical field. Identity Crisis never copies recursively, and private awareness or audience permissions never transfer. In Participant play, the player-controlled Prosecutor is eligible. In court, a witness holder retargets to whoever is currently speaking directly to that witness for the exchange. Identity Mirror presentation has precedence over Shapeshifter. The target form, copied public Power consequences, direct-address event, and timing are frozen for replay, and gameplay never calls an LLM or synthesizes a new line.",
  );

WHODUNNIT_TUTORIAL_STEP.body = `Whodunnit setup is a guided five-step path: Experience, Mansion, Story, Production, then Cast. Only the current step's decisions are shown; Back and Continue preserve the draft, and the progress row lets you revisit an earlier choice. Experience first establishes the court tone and whether you investigate the mansion before trial. Mansion then asks whether to use Installed Mansions or create a new Quick, Standard, Grand, or Custom house. Quick, Standard, and Grand show their matching PRISM House exterior covers; Custom uses a question-mark cover until its floor and room choices determine the exterior scale family. Quick uses the compact exterior family, Standard uses standard, and Grand uses grand; Custom derives its family from the frozen public floor and room count. Choose Use this mansion for a new case, or let Random installed mansion pick another installed house without repeating the current one when alternatives exist. Each mansion card carries a quiet origin badge: Imported means it came from a portable .mansion package, Created here means it was saved from a PRISM mansion level, and Derived marks an editable local copy. Every card uses one high-quality exterior establishing cover that shows the complete mansion in its geography; interiors and room mosaics are never cover fallbacks. Edit details opens a contained dialog for the local library exterior, title, and description; each field can return to its package or original default. Duplicate & edit mansion preserves that source, creates a Derived copy, and opens the full-screen Mansion Editor. Its fixed 16 by 12 planner keeps each room type's existing footprint and silhouette. Each semantic room type can be placed only once across the whole mansion, regardless of floor; placed types remain visible but unavailable in both the palette and room inspector. Drag blocks to arrange the house: collisions move nearby blocks to the closest legal connected position, while only a move that creates an island returns. Double-click any room or corridor to rotate it. Corridors are traversable, resizable circulation blocks that shape the house without becoming semantic rooms or changing house scale; resize one directly by dragging its edge. Shared-wall doors appear as thick yellow marks, auto-center when a block joins the plan, then can slide, be removed, or be placed again on a valid shared wall; room neighbor compatibility comes from this geometry instead of authoring checkboxes. Select a room and choose Open Room Editor to drill in, then use the breadcrumb or Back to return. New rooms begin with bundled room-type art, while Mosaic is preview-only. Choose Place anchor, then click the room image to add named on, in, under, behind, beside, or near authoring context—not hotspots—with at most twenty-four per room. Each room can carry at most eight seeded fire, omni, directional, or freehand-neon lights; place them on the room, drag to move, and use the edge handle to resize. Reduced Motion freezes their deterministic animation, and cue permissions are metadata for a static mansion presentation in this release. In ONLINE, Generate room-art candidate stages mansion-owned art without replacing the accepted plate until Accept candidate; Retry candidate and Discard candidate also leave accepted art intact. LOCAL disables generation and continues with bundled or already accepted art. Floor 3 stays disabled until Floor 2 has at least four semantic rooms. Save requires an occupied upstairs, a functional foyer staircase, no overlaps, accessible semantic rooms, and one connected walkable plan; the saved derivative becomes the mansion selected for this case. Install a mansion file opens a separate inspection dialog instead of expanding setup. Story holds the optional Mystery spark and difficulty. As you type, PRISM heard chips reflect only public motifs such as Grand theft, Inheritance, or Stormbound; they never reveal responsible parties or sealed relationships. The first explicit incident motif becomes the primary charge; otherwise the recipe seed may choose homicide, theft, fraud, sabotage, espionage, disappearance, or blackmail. Casual keeps only that primary incident, Classic may compose one complication, and Mastermind may compose two. Leaving the spark blank creates a seeded surprise. Theory Board names one or two accused people against the public charge; Court keeps Press and Present cross-examination, then returns a Guilty or Not Guilty result for each defendant. Production holds custom art, audio, recipe controls, and legacy Case Seed import. Cast begins with the Jury Trial toggle; the four juror seats appear only while it is on, and Bench Trial hides them without changing the Judge or courtroom cast. Select a role card first, then choose a Library bot to assign that exact role; clicking a role card never changes its current bot. Hold the Wield modifier and click a cast container to open a prompt-free group picker; choose a Library group to reroll only that container, and choose again to reroll while it stays open. Wield an individual seat or Library bot tile for a prompt-free random bot replacement. Those Cast Refract paths remain prompt-free and never change sealed case truth. ${WHODUNNIT_TUTORIAL_STEP.body}`;
WHODUNNIT_TUTORIAL_STEP.body = `Installed Mansion details owns one Soundscape surface with separate Music and Atmosphere tabs. Use either inline player to audition the packaged layer or PRISM fallback. ONLINE can synthesize an instrument-only investigation theme made of brief noir phrases and long quiet intervals; environmental sound belongs only to Atmosphere. PRISM decodes each music candidate, requires substantial near-silence and quiet loop boundaries, then stages it until you choose Use this version. The Atmosphere tab can synthesize one seamless mansion-wide environmental bed while room profiles continue to shape exposure, filtering, emitters, and speech ducking without generating a separate mix per room. Discard or a failed generation leaves the active layer untouched, and Undo previous version restores one prior accepted layer. LOCAL can play packaged and fallback music and ambience but never contacts ElevenLabs. ${WHODUNNIT_TUTORIAL_STEP.body}`;
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body
  .replace(
    ...
    "Mansion then asks whether to use Installed Mansions or create a new Quick, Standard, Grand, or Custom house. Create a new mansion also offers Open Mansion Editor: it creates a tenant-owned blank draft with the smallest valid connected two-floor plan, then opens the same full-screen editor used for Derived mansions. The complete semantic room-block palette runs from Ballroom through Garage. Foyer, Basement, and Garage stay on Floor 1; Attic and Rooftop Lounge stay on the highest occupied floor. The palette and room inspector keep impossible choices visible but disabled, and saved validation enforces the same rules. Select any removable room or corridor and remove it from the right inspector. Continue checks the connected plan and derives its bundled or accepted 24-color Mosaic silhouettes behind an informative full-screen blocker, then returns to the mansion map so you can review it before choosing Use this mansion.",
  )
  .replace(
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body
  ...
  .replace(
    "Before compiling, Theme / Spark can steer the story, atmosphere, era, and visual language; Surprise me remains a valid empty direction. Skip investigation is an optional frozen setup choice: it compiles only the court act and its reachable dialogue, begins directly in court, leaves mansion prop-pack completion optional, and disables Room and investigation-Music assets. The same frozen one-house contract keeps every current and future room and prop asset coherent. Case Forge can prioritize the functional prop roles this case needs, then complete the mansion’s reusable 16-role pack in the background; ready mansion variants and PRISM fallbacks keep play available immediately. Starting immediately creates a durable case and opens the spoiler-free Case Forge: Writing the Case, Testing Contradictions, Directing Performances, Preparing Local Voices, Verifying Case Audio, then Begin Case. Its animated checkpoint bar advances only when a durable section completes, while the Current work list restores the saved foundation, room-detail, witness, deterministic-check, or local-recording step after you return. Court-only cases omit room-detail work and use court chapters instead. Elapsed time updates live, an approximate ETA appears after enough completed sections exist, and calm Forge notes rotate visually without being repeatedly announced to assistive technology. You may Archive during preparation, quit the app, or otherwise leave and return to continue from the last lit durable checkpoint without rebuilding verified work.",
    "Mansion owns the exterior before Story: a new mansion can explicitly Refract an ONLINE exterior with player-authored direction, while LOCAL retains bundled art and never contacts an image provider. The accepted draft freezes into the case and appears in Case Forge, the title card, Archive, and previews. Mansion idea and Story may dress the premise and tone but never silently replace the selected house, scale, or geography. Participant setup offers Skip investigation, while Spectator setup calls the same direct-court choice Start directly in court; either compiles only the reachable court act, bypasses Theory Board review, keeps Evidence image synthesis optional in ONLINE, and disables Room, Music, and Ambience preparation. In ONLINE, Evidence and Rooms are opt-in: Evidence creates sealed exhibit art, while Rooms creates sealed room edits without regenerating the chosen exterior. In LOCAL, Evidence image synthesis is unavailable: authored evidence remains fully playable through text and symbolic evidence cards, while the title card uses the bundled or installed mansion exterior and rooms use bundled PRISM art or artwork already installed with the mansion. Starting creates a durable case. Participant Case Forge retains its detailed six stages: Writing the Case, Testing Contradictions, Directing Performances, Preparing Local Voices, Verifying Case Audio, then Begin Case. Spectator Case Forge instead says Preparing your mystery to watch and groups the same five durable backend passes into Writing the trial, Checking the case, and Recording the cast. Progress, elapsed time, ETA, and Archive remain visible; raw stage messages, substeps, attempt count, recording totals, and the local-voice notice live in collapsed Preparation details. Failure expands the recovery information and keeps Retry, silent continuation when eligible, error-copying, and Return to setup available. You may Archive during preparation, quit the app, or otherwise leave and return to the last durable checkpoint without rebuilding verified work.",
  );
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body
  .replace(
    ...
    "In a Participant case, Begin Case first presents the mansion's high-quality exterior establishing cover behind the title card, then opens inside the central incident room beneath the Casekeeper briefing. Continue into one finite visible sweep; Move unlocks only after every visible incident-scene point is reviewed.",
  )
  .replace(
    "Move returns to PRISM’s shallow-isometric mansion and enters any unlocked room for free.",
