  ...
  debate: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "In Basic, describe the idea in ordinary words and choose Build the debate. Prism makes one balanced motion, names both sides, and writes the briefs you should not have to author; Try another version gives you a fresh framing. Across Territory / Your idea, Prompt Center prompts and wildcard rolls insert as ordinary editable text—the same Command Center powers as Signal setup—and any leftover placeholders resolve when you build or Refract. The floating Prism remains available throughout setup and understands the bounded, unsaved workbench draft. Wield Prism into a glowing setup field—or use your Prism modifier + Space while that field is focused—for a contextual editable candidate shaped by the current format, Rowdiness, role, cast, motion, and evidence. Space rerolls; Enter, Tab, or clicking another input accepts; Escape or a non-input outside click restores the original. Advanced restores three motion choices and direct editing of every field. The territory dice remains available for a fast local catalog seed.",
