...
WHODUNNIT_TUTORIAL_STEP.body = `Whodunnit setup starts with the shortest complete path: choose Quick, Standard, Grand, or Custom; leave Mansion idea blank for Surprise me or add one optional direction; keep Classic for balanced deduction; then choose the cast. A complete mystery does not require opening More case options. Installed Mansions is a dedicated library: choose Use this mansion for a new case, or let Random installed mansion pick another installed house without repeating the current one when alternatives exist. Edit details changes only the local library thumbnail, title, and description; each field can return to the immutable .mansion file default. Install a mansion file imports a shared .mansion into that library, while court tone, direct-to-court play, custom production, and deterministic recipe controls live under More case options. ${WHODUNNIT_TUTORIAL_STEP.body}`;

WHODUNNIT_TUTORIAL_STEP.body +=
  " Mansion packages are optional: setup can open or accept a dropped .mansion, show its creator, protection, compatibility, size, provenance, license, content notes, room map, and theme status before anything is installed, then use it for a fresh case entirely offline. A fully explored saved mansion can be exported with PRISM’s automatic spoiler seal or an optional password; imported work can be re-exported only when its license permits. Music and Ambience are separate Case Forge choices. Ambience on freezes a mansion-specific environmental identity; in LOCAL it stays procedural and shared with no online request. Ambience off still uses the bundled theme palette and room-acoustic rules, so global Audio—not this setup choice—is the silence control. During Investigation the continuous world bed crossfades its room mix without restarting, ducks under speech, and imported mansions retain embedded ambience with semantic-role fallbacks.";
