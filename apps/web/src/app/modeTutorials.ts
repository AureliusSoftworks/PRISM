...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body.replace(
  ...
  "Examine is a first-person room-art viewing mode: a lens cursor quietly confirms an available inspectable region only while it is inside that region; click there or focus it with the keyboard to inspect. While Examine is open, Mosaic’s fine tessera separators remain visible and a flat, screen-aligned 24×15 search grid appears over both Mosaic and Upgraded room art. When the lens is inside an available region, exactly the square beneath the lens glows; reviewed and locked regions stay quiet. Examine and Move each slide the Case Desk away and remove the room vignette so the active surface can fill the screen. Back to room exits either focused mode; leaving Examine clears its search grid and restores the vignette and Case Desk while preserving the Mosaic separators;",
);
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body.replace(
  ...
  "Ordinary spoken Investigation dialogue closes itself after its audio and caption hold. Written room observations remain open after their final character until you click a non-interactive part of the screen or press Space or Return. A choice, evidence response, examination selection, tutorial acknowledgment, or other meaningful prompt also stays open until you answer it.",
);
