  ...
  debate: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "When you are the Judge, Gavel stays available throughout the live proceeding, and Space swings it without reaching for the button. During an advocate’s overtime it calls time and immediately restores the scheduled order. At any other point it calls the room to order and opens one message to both advocates; Prism routes the advocate best positioned to answer once, then resumes the exact interrupted schedule. Judge questions and interventions replace the caption with three one-tap responses and a fourth Write my own choice. Write my own reveals the standard composer at the bottom: type there, or use its dice to draft editable Judge prose before sending. Final rulings stay one-tap. For two seconds after the first strike, hammer Space as much as you like: those extra smashes are pure showmanship and never create another message, answer, ruling, or schedule change. The eight-second server cooldown still prevents a second deliberation.",
        clickLabel: "Gavel",
