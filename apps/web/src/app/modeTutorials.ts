  ...
  botcast: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Signal automatically compiles a director’s-cut video from the finished transcript, saved production events, and the frozen voice used for each line; it never asks an AI to recreate the episode. Replay restores the full transcript beside the saved camera cut and gives you standard video controls, captions, transcript-line seeking, video and transcript downloads, Retry when a frozen voice is unavailable, and Delete Recording without deleting the episode transcript. Voice changes begin on the same next line they affected live, crosstalk keeps two caption lanes, and closing PRISM leaves the durable job queued until a capable client reopens it. There are no post-episode camera controls. At the end card or in replay, Copy for Signal Review puts the complete conversation plus its private cues, per-turn model routing, delivery notes, segment changes, camera decisions, and outcome on your clipboard for a focused review; Delete episode remains the separate destructive action.",
...
export const MODE_TUTORIALS: Record<TutorialMode, ModeTutorial> = {
  ...
  coffee: {
    ...
    steps: BASE_MODE_TUTORIALS.coffee.steps.map((step, index) => {
      ...
      return index === 0
        ? { ...step, body: `${body} ${POWER_EXCLUSION_TUTORIAL_SUFFIX} ${IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX}` }
        : body === step.body
          ? step
          : { ...step, body };
    }),
  ...
  botcast: {
    ...
    steps: BASE_MODE_TUTORIALS.botcast.steps.map((step, index) => {
      ...
      return index === 5
        ? {
            ...step,
            body: `${body} ${SIGNAL_AVATAR_SCALE_POWER_TUTORIAL_SUFFIX} ${POWER_EXCLUSION_TUTORIAL_SUFFIX} ${SIGNAL_ADDRESSED_FANDOM_POWER_TUTORIAL_SUFFIX} ${IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX} ${SIGNAL_PRODUCER_GUEST_TUTORIAL_SUFFIX}`,
          }
        : body === step.body
          ? step
          : { ...step, body };
