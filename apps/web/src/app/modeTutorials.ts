  ...
  zen: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "AUTO keeps the current response model as Primary, then quietly tries your ordered chain of one to five saved local or online fallbacks if a reply fails validation. While AUTO is selected, the model picker shows every model so you can change Primary directly. Image generation keeps its own LOCAL/ONLINE choice in Images. Chat itself is deliberately text-only: its Voice selector stays locked to Mute, entering Chat stops foreground speech, and neither player nor bot messages request voice synthesis. This contextual mute never overwrites your saved Voice preference; English, Premium, Babble, or Bottish resumes unchanged in Coffee, Signal, Debate, Story, and other voiced surfaces.",
  ...
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Avatar Studio and Zen player voice settings use a Pronunciation Atlas above the familiar adjustment grid. Its clean, unlabeled holographic world projection maps American and British English foundations plus every qualified approximate Speechprint without cluttering the Earth with region nodes. Drag the pin across the map or use arrow keys to choose a region, then set Light, Balanced, or Strong influence; the collapsed List view offers the same controls without the map. Genuine accent source and Vocal presentation still filter the voice picker to matching identities. The atlas changes only private local phonemes: Auto uses Instant to honor them, while forced Voice+ and installed operating-system voices show them as suspended. Source and Current previews compare the result without changing visible dialogue, memory, or exports.",
        clickLabel: "the LOCAL, AUTO, ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
...
function currentDebateJuryTutorialBody(body: string): string {
  return body
    ...
    .replace(
      ...
      "For a returning human Judge, Resume is the return-to-order gavel strike: one click swings the visible gavel, sounds its hit, settles the room, and continues without asking for a second slam. Other roles resume silently. If a spoken line was interrupted, Debate replays that exact saved line from its beginning before continuing.",
    )
    .replace(
    ...
    .replace(
      ...
      "Pause and Resume remain lifecycle state only. The Judge's Resume gavel is audiovisual room control, not a spoken beat or transcript event.",
    )
    .replace(
    ...
    .replace(
      ...
      "Once Jury deliberation begins, the Jury owns the floor: the unified Gavel, Space, End, and Skip actions are put away until every ballot is complete. Pause remains silent; when a human Judge resumes, that Resume is the audiovisual return-to-order gavel.",
    )
    .replace(
