...
WHODUNNIT_TUTORIAL_STEP_SOURCE.body = WHODUNNIT_TUTORIAL_STEP_SOURCE.body
  .replace(
    ...
    "Choose Whodunnit? in Debate format and stay inside Debate Studio's existing Court and Cast rails. On Court, choose Participant to prosecute your filed case or Spectator to watch both counsel bots argue it; Participant keeps Coach, Standard, and Immersive feedback choices. Player Judge remains visible but unavailable: the Cast page now separates the case ensemble from the public court, where you choose a distinct Judge bot alongside Prosecution and Defense. That Judge sees and rules on the frozen public record while PRISM remains the sealed server-side Casekeeper. Set the shared five-stop court formality and choose whether the separately grouped standard Debate Jury deliberates. On Cast, use Debate's Library selector, search, and groups to seat four to eight suspects, the public Judge, a prosecutor partner, and a rival defense bot; Surprise me and Wield Prism populate every distinct seat while keeping the result editable before you compile a fictional, non-canonical Case Seed.",
  )
  .replace(
    "In play, the compact dark HUD names your mission and keeps Actions, Case file, and Theory reachable.",
    "At the opening case assignment, choose Investigate the mansion to own every discovery, or Trust your partner to skip the mansion and jump straight into Turnabout. The partner route deterministically files one court-worthy public case from the sealed Case Bible: you inherit only its discovered evidence, witnesses, and accusation, the mansion closes immediately, and the courtroom still makes the prosecution prove that record through testimony and cross-examination. In manual play, the compact dark HUD names your mission and keeps Actions, Case file, and Theory reachable.",
  )
  .replace(
        ...
        steps: tutorial.steps
          ...
          .map((step, index) => {
            ...
            const accentContinuumTutorialStep =
              currentStep.heading === "Shape an offline voice"
                ? {
                    ...currentStep,
              ...
              mode === "botcast"
                ? {
                    ...
                    body: neutralSignalComposerCopy.body
                      ...
                      .replace(
                        "Interrupt guest now plays one of that host’s saved short interjections immediately while a meaningful cutoff can overlap with the guest’s annoyed ending and the host’s continuation generates.",
                        "Interrupt guest now keeps the guest talking while one of that host’s saved short interjections prepares. When the host voice actually starts, the guest trails off through a brief overlap and the saved audience-heard cutoff is fixed at that instant while the host’s continuation generates.",
                      )
                      .replace(
            ...
            const coffeeRosterTutorialStep =
              mode === "coffee" &&
              audibleHandoffTutorialStep.heading === "Pick or stage your table"
                ? {
                    ...audibleHandoffTutorialStep,
                    body: audibleHandoffTutorialStep.body.replace(
