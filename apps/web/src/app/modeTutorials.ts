  ...
  avatar: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Identity, Eyes, Mouth, Details, Profile, Powers, Voice, SFX, and Settings remain available in the perimeter dock. On Identity, use Core for name and thinking, or Shell for the hue strip and badge. The top LOCAL/ONLINE toggle and model picker choose which model compiles Powers—switch to ONLINE when a local compile fails. Prism stays unavailable inside this main panel. Everything stays a draft until the top Save or Create bot action.",
...
function currentDebateRecessTutorialBody(body: string): string {
  return body
    ...
    .replace(
      "Leaving an unfinished Debate by any route preserves its exact floor and makes the returning chamber appear in recess. Continuing begins with the moderator calling the Debate back to order unless the Jury chamber is visible.",
      "Leaving an unfinished Debate by any route preserves its exact floor and makes the returning chamber appear in recess. Continuing always begins with the Moderator calling the Debate back to order, including from the Jury chamber.",
    )
    .replace(
...
function currentLocalOnlySimulatedEffortTutorialBody(body: string): string {
  return currentChatZenPresentationTutorialBody(body)
    .replace(
      "private simulated passes may still run underneath for quality.",
      "LOCAL simulated passes may still run underneath for quality.",
    )
    .replace(
      "On models without built-in thinking, Effort runs Prism’s simulated private passes;",
      "On LOCAL models without built-in thinking, Effort runs Prism’s simulated private passes;",
    )
    .replace(
      "Those passes guide the final reply; with an online model, each one is an additional provider request.",
      "Those passes guide the final reply; every simulated pass stays in the LOCAL lane and uses the selected Ollama model. Prism does not simulate thinking for ONLINE models, which use only provider-native effort.",
    )
    .replace(
      "Models without a built-in thinking dial always get Prism’s simulated Effort (None through Extra High):",
      "LOCAL models without a built-in thinking dial get Prism’s simulated Effort (None through Extra High):",
    )
    .replace(
      "Settings → Experimental can optionally enable Deep simulated thinking for a much heavier private workshop.",
      "Settings → Experimental can optionally enable Deep LOCAL simulated thinking for a much heavier private workshop.",
    )
    .replace(
      "Online simulation may add provider usage or cost; native effort remains native.",
      "Every simulated pass stays in the LOCAL lane; ONLINE models use only provider-native effort.",
    );
}
...
function currentModelRoutingTutorialStep(
  ...
  const body = currentLocalOnlySimulatedEffortTutorialBody(step.body);
  if (
    !/model/iu.test(body) ||
  ...
  return {
    ...step,
    body: currentLocalOnlySimulatedEffortTutorialBody(
