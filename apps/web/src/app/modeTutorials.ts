    ...
    steps: [
      {
        ...
        body: "Chassis lights share one accent color and stay dark on a new manual shell until each section has meaningful setup; generated drafts energize the modules they already populated. Before generation, Model can follow Auto or target one model for that draft without changing your account default. Start from the perimeter dock—open Eyes first, then Mouth, Identity (Core or Shell), Ink Display, and the rest of the foundry tabs.",
  ...
  zen: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "When a saved group is selected, Atmosphere keeps its reusable backdrop behind the standard bot grid. The leading + uploads; wield Prism onto that same tile to synthesize. Recent group-room Atmospheres sit beside it, and View all opens the exact-type local library. In asset details, Reduce magenta applies one cumulative local color-key pass to the whole asset set; Undo last pass restores the preceding revision. Marketplace groups keep their stable bundled scene until you replace it.",
  ...
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Right-click in the canvas for shortcuts to settings, memories, images, and bot actions. In Settings → Storage, Space Lens includes Audio for your synthesized Sound Effects and Music (bot action packs, avatar loops, Signal idents and atmosphere beds). Smart tidy clears abandoned library assets into recovery trash without listing every file unless you ask to review samples. In the account-wide Images hub, type the reusable prompt, then select Synthesize. Five recent general images stay in the rail and View all searches only general images, never Slate covers, Signal studios, logos, exhibits, or Atmospheres. Asset details can apply a cumulative local Reduce magenta pass and a separate Compress size control, each with Undo. Uploads appear only where an asset is actually needed, such as a cover, studio, exhibit, or Atmosphere. A running render can still queue up to eight more prompts with its captured model, keywords, privacy, and library.",
        clickLabel: "the conversation canvas with your right mouse button",
        targetSelector: '[data-tutorial-target="conversation-canvas"]',
      },
      {
        heading: "Keep the moment honest",
        body: "Zen keeps the timeline as it happened. Type $undo to rewind the latest message when you need a clean correction. Put physical stage direction in the separate Action field using letters and spaces only; typing exactly ** in the speech field jumps there. Action drafts stay private until Send. If you send an Action without speech, it and the bot's action response appear on the canvas as an ephemeral exchange and never enter history or memory. Action text stays visual and is never read aloud as dialogue. With your Premium player voice, a recognized vocal Action such as laughs becomes an ElevenLabs performance direction for that line. PRISM-only fart, burp, and cough actions stay out of the voice request and play their bundled local Foley when sent, just as they do in Coffee, Signal, and Debate. For a surprise line, Wield Prism onto the message box the same way as elsewhere—no separate dice control. Chat and immersive Zen wait for the real reply instead of inserting a filler response. When Shh appears, it stops the current reply without replacing the draft you are writing.",
        clickLabel: "the message box at the bottom",
        targetSelector: '[data-tutorial-target="composer"]',
      },
      {
        heading: "Shape an offline voice",
        body: "Choose LOCAL or ONLINE as the hard network privacy lane. Private chat is separate: enable it from the new-chat hero when you do not want the conversation or memories saved. Once that conversation begins, a locked Private chat badge remains in the navbar as status, not a switch. Switching to another applet disarms Private chat, so returning Home never starts privately by surprise. Avatar Studio Voice uses Accent, Feel, and Voice stages: Accent keeps a full-width map, Feel tunes character and performance, and Voice chooses who speaks. Zen player voice settings keep the Accent map. Click or drag anywhere in the world: the pin stays exactly where you leave it while Prism approximates the nearest broadly regional pronunciation—including New York and Southern U.S.—then set Light, Balanced, or Strong. All accents opens the exact list. The Gender filter mixes American and British base voices in one catalog; choosing a voice keeps its genuine source while the map shapes its private pronunciation without changing dialogue text, memory, or exports. Original and With accent let you compare the result.",
        clickLabel: "the LOCAL / ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
...
function currentSignalRefractTutorialBody(body: string, index: number): string {
  const current = body
    ...
    .replace(
      ...
      "Refract replaces the old Topic and Private comments dice: hold Option on macOS or Control on Windows and Linux to Wield Prism, then click either glowing registered field. You can also focus it and press your Summon / Wield Prism shortcut—Control Space by default—or drag the Prism orb onto it. Space rerolls after a draft settles. Clicking away, Enter, Tab, or clicking another input keeps the current draft. Option-clicking a different registered control on macOS or Control-clicking a different registered control on Windows and Linux also keeps the current draft, then moves Prism into that new control. Escape restores the original. Shortcuts can be changed in Settings → Shortcuts.",
    );
  if (index === 0) {
