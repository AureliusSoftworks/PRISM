...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body.replace(
  ...
  "Examine is a first-person room-art viewing mode: a lens cursor quietly confirms an available inspectable region only while it is inside that region; click there or focus it with the keyboard to inspect. While Examine is open, Mosaic’s fine tessera separators are hidden and a flat, screen-aligned 24×15 grid appears over both Mosaic and Upgraded room art. When the lens is inside an available region, exactly the square beneath the lens glows; reviewed and locked regions stay quiet. Examine and Move each slide the Case Desk away and remove the room vignette so the active surface can fill the screen. Back to room exits either focused mode; leaving Examine also clears its search grid and restores the Mosaic separators, vignette, and Case Desk;",
);
                ...
                ? {
                    ...
                    body: compactAvatarSpeechStep.body
                      ...
                      .replace(
                        ...
                        "Signal accepts exactly one PNG, JPEG, or WebP image per episode. The filename stem supplies its name. Every upload is treated as a generic image and appears in a light or dark Polaroid frame that follows the active theme; transparency never turns it into an Item. Images use the same compact camera placement:",
                      )
                      .replace(
                ...
                ? {
                    ...
                    body: neutralSignalComposerCopy.body
                      ...
                      .replace(
                        ...
                        "When a private Host note, direct Say this cue, or attached image arrives while the host is audibly speaking, Signal cuts at the live voice clock and the host pivots on mic. A cut in the pause between words leaves an ellipsis; an active-speech cut leaves a sharper dash. The saved pivot may add a brief hesitation, self-correction, breath, or throat clear, and replay uses that same performance. Other early control-card cues can still redirect naturally; once most of the point is already out, they stay queued for the host’s next turn.",
                      )
                      .replace(
                ...
                ? {
                    ...
                    body: audibleHandoffTutorialStep.body
                      .replace(
                        ...
                        "one PNG, JPEG, or WebP image before recording",
                      )
                      .replace(
