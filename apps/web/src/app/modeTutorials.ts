  ...
  botcast: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Signal records the real broadcast live from ident through outdent, preserving its actual voice audio, timing, captions, camera choices, interruptions, producer cuts, and frozen cast appearance without another AI turn. The video is normally ready within seconds of the closing card. If capture is interrupted by a hidden tab, reload, browser encoder problem, or audio discontinuity, Signal quietly rebuilds the episode from the same frozen source material and production events instead of presenting a partial recording; the episode card shows progress while that recovery runs. Select the archived episode after Video ready to open the browser-playable video. You can play, pause, scrub, download the video or transcript, Retry when a frozen voice is unavailable, and Delete Recording without deleting the episode transcript. There are no post-episode camera controls and no recording toggle. At the end card or in the saved episode, Copy for Signal Review puts the complete conversation plus its private cues, per-turn model routing, delivery notes, segment changes, camera decisions, and outcome on your clipboard for a focused review. Delete episode sits beside it only after the broadcast is complete and remains the separate destructive action.",
...
const BOT_NAMING_POWER_TUTORIAL_SUFFIX =
  "A bot-name prefix or suffix changes only how its holder names other bots: the holder keeps their own name, the player and humans are untouched, and other speakers do not copy the habit.";
