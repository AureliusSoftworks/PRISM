  ...
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Choose LOCAL or ONLINE as the hard network privacy lane. Private chat is separate: enable it from the new-chat hero when you do not want the conversation or memories saved. Once that conversation begins, a locked Private chat badge remains in the navbar as status, not a switch. Switching to another applet disarms Private chat, so returning Home never starts privately by surprise. Avatar Studio Voice uses clear Accent, Local, and Premium stages: first place the required Accent pin, then shape the Local voice and Feel, then the Premium voice and Feel. One pin controls both engines, so changing from Pia to Rowan—or any other named voice—does not move the bot across regions. Premium and Local share one target-pronunciation path; the pin never changes the spoken language. Zen player voice settings keep the full-width map in Accent. Click or drag anywhere in the world: the pin stays exactly where you leave it while nearby regional features blend gradually, including denser London, U.S., and European detail. Nearby choices expose named local variants such as Cockney without guessing identity from location; All accents opens the exact full list. Moving, dropping, or choosing a pin is always silent. To hear a bot, use only the Local, Premium, or Speak buttons beneath that bot in its preview panel. Then set Light, Balanced, or Strong. Voice range filters the named catalog without exposing engine regions; dialogue text, memory, and exports stay unchanged.",
        clickLabel: "the LOCAL / ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
...
const DEBATE_UNINTELLIGIBLE_FLOOR_TUTORIAL_SUFFIX =
  "If a public advocate line is unintelligible—mumbled, garbled, or otherwise not a recognizable argument—the bot chair cuts in after that line lands, notes that it cannot stand, and continues the docket. It does not cut the speaker mid-word. When you are the Judge, PRISM never takes that call from you.";
