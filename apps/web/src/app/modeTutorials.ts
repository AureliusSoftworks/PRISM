...
const IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX =
  "An identity-mirroring bot visually borrows the latest eligible direct addresser’s exact eyes and blink package, complete resting and speaking mouth package including glyph style and Custom Speech poses, authored Avatar Details Ink, lower glyph, and a literally double-quoted copy of the target’s public name. Its color, complete authored voice and Accent Map location, pronunciation, Speechprint, provider voice identity, chassis/frame, Powers and consequences, thinking spinner, bot ID, seat, role, private perception, safety, provider, and every other speech or mechanical field remain its own. The screen briefly powers down while the overlay is installed, then reveals it. The saved handoff and timing replay exactly and reset with the session. Coffee and Signal target bots; Whodunnit V2 can also target its player-controlled Prosecutor.";
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body
  ...
  .replace(
    ...
    "Move returns to PRISM’s shallow-isometric mansion and enters any unlocked room for free. Unvisited rooms reveal no occupant glyph. On first entry, the room opens clean and unblurred with no commands; the Casekeeper shows a silent \"...\" until you click. The room then softens, the suspect appears, and their frozen persona introduction plays from the prepared local pack with mouth motion and a separate stage action. Controls return only after that exact performance ends; revisits skip it. After every room is unlocked and visited and every examination point is reviewed, Save mansion level preserves the layout, house direction, and room assets as one protected level for later Whodunnit setup. Storage cleanup cannot remove an image still owned by that saved mansion.",
  )
  .replace(
  ...
  .replace(
    ...
    "Identity Crisis lets the holder knowingly masquerade as the latest eligible direct addresser by taking their exact eyes and blink package, complete resting and speaking mouth package including glyph style and Custom Speech poses, Avatar Details Ink, lower glyph, and literally double-quoted public name. The holder defensively treats the original as the suspicious imitator, with mild concern rather than panic or constant repetition. In Participant play, the player-controlled Prosecutor is eligible. The holder’s color, complete voice and Accent Map location, pronunciation, Speechprint, provider voice identity, chassis/frame, Powers, thinking spinner, and every other speech or mechanical field remain unchanged. The target form, direct-address event, and timing are frozen for replay, and gameplay never calls an LLM or synthesizes a new line.",
  );
