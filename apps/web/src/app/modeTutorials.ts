  ...
  if (applet === "Signal") {
    return body
      ...
      .replace(
        ...
        "Routing, model, Effort, Voice, Settings, Usage, Memories, Images, Bots, and app switching stay locked through the closing card until you Return to show. The compact Light/Dark control remains in the live topline and changes only PRISM’s appearance; it never alters the episode, recording, or replay.",
      )
      .replace(
        ...
        "routing, Settings, Usage, Memories, Images, Bots, and app switching stay closed through the closing card until you Return to show. The compact Light/Dark control remains in the live topline and changes only PRISM’s appearance; it never alters the episode, recording, or replay.",
      );
  }
  return body
    .replace(
...
WHODUNNIT_TUTORIAL_STEP.body = WHODUNNIT_TUTORIAL_STEP.body
  ...
  .replace(
    ...
    "Inside the open Room Editor, Regenerate room asset deliberately returns only that room to its bundled Pixel Art plate and clears that room's anchors, lights, and staged art. Synthesize Pixel Art · ONLINE authors a new Pixel Art candidate for only the open room, using an existing high-resolution room as a strict composition reference when one is available. Pixel Art is a separate authored asset, never a filtered preview. Its Mosaic preview adds PRISM's balanced, hue-preserving grid without baking that grid into the accepted source. The source and accepted plate remain intact until Accept candidate; retrying or discarding also leaves them intact. LOCAL cannot synthesize new room art and continues with bundled or already accepted art.",
  );
...
WHODUNNIT_TUTORIAL_STEP.body +=
  " Investigation room art defaults to genuine synthesized Pixel Art with one shared Mosaic presentation: one 320×180 logical source sample owns each complete tessera, then lossless nearest-neighbor scaling and an ordinary Normal-blend separator grid preserve crisp, indivisible cells with no high-resolution detail showing through underneath. Each scene's own light balance darkens bright separators and lifts dark separators without washing out its color. The authored source stays gridless. Existing high-resolution rooms act only as strict composition references: Pixel Art is newly authored with deliberate pixel clusters, while the source remains unchanged. Future room generation creates Pixel Art first; Upgrade to Realistic · ONLINE later derives its materials, photographic depth, and smooth edges from the clean accepted Pixel Art plate without changing doors, stairs, floor paths, hotspots, anchors, lights, evidence geometry, movement, dialogue, audio, or Court. LOCAL never sends a room to a remote generator. Once every requested Realistic plate is ready, switch styles at any time. Pixel Art uses Mini avatars and Realistic uses Full avatars at the same stage footprint. Portable mansions identify synthesized Pixel Art as the primary room asset, record the presentation-only grid contract, and carry an optional Realistic asset when one exists.";
WHODUNNIT_TUTORIAL_STEP.body +=
