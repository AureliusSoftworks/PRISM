...
const BASE_MODE_TUTORIALS: Record<TutorialMode, ModeTutorial> = {
  zen: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "AI-generated and Marketplace bots ask ElevenLabs for a fresh “Computer calculating” loop and assign it only to thinking. If LOCAL privacy or an unavailable ElevenLabs key blocks that request, the bot draft or install still completes and the SFX tab remains available for a manual retry.",
        clickLabel: "the LOCAL, AUTO, ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
      {
  ...
  botcast: {
    ...
    steps: [
      ...
      {
        heading: "Replay or export video",
        body: "Signal captures the actual voice takes, camera choices, captions, production events, departures, soundboard timing, and frozen cast appearance while the episode is live; it never asks an AI to recreate what happened. Selecting a completed episode opens its local replay immediately with no render gate or new voice request. Export video renders the saved studio visuals in the background, records that visual performance silently, and then adds the voice takes and studio audio captured with the episode, so it makes no future LLM or ElevenLabs call. Local replay remains available before and after export. Export Premium video is a separate explicit ONLINE action: after you confirm that the exact spoken transcript and selected voice IDs may be sent to ElevenLabs and consume credits, Signal creates a cached Eleven v3 dialogue master and renders a separate Premium cut without replacing the standard export. The episode card shows each export's progress independently. A Premium video failure can retry from cached audio without another paid voice generation. There are no post-episode camera controls and no second AI conversation turn. Copy for Signal Review still puts the complete conversation plus its private cues, per-turn model routing, delivery notes, segment changes, camera decisions, and outcome on your clipboard. Delete episode remains a separate destructive action.",
