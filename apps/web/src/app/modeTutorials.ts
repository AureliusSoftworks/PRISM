...
const IDENTITY_MIRROR_POWER_TUTORIAL_SUFFIX =
  "An identity-mirroring bot copies the public persona, CRT face, and resolved voice of the latest bot who directly addresses it. It announces the stolen identity and calls the original an impostor once, then inhabits that persona without repeating the claim; its own glyph, color, role, Powers, private state, and hard speech restrictions remain anchored, and the player is never copied. The saved face-and-voice handoff replays exactly and resets with the session.";
...
function currentSignalPowerTutorialBody(step: ModeTutorialStep): string {
  return currentInterruptionRetortTutorialBody(step.body)
    .replace(
      ...
      "If the host is muted, Signal records one opening ellipsis, then the bot guest carries a self-directed solo broadcast through the topic and spoken closing instead of answering imaginary host questions.",
    )
    .replace(
