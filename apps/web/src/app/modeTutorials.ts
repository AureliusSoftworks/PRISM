...
const BASE_MODE_TUTORIALS: Record<TutorialMode, ModeTutorial> = {
  ...
  debate: {
    ...
    steps: [
      ...
      {
        heading: "Start from a proceeding preset",
        body: "University Union, Daytime Showdown, Crossfire, Town Hall, and Bench Trial each set the room’s format, formality, role, and Jury stance. Motion, cast, evidence, provider, model, participant side, and stage alignment stay yours. A preset clears advocacy consent only when it changes format or formality; any manual difference becomes Custom. Presets are built in and are not saved or edited.",
        clickLabel: "a proceeding preset",
        targetSelector: '[data-tutorial-target="debate-presets"]',
      },
      {
        heading: "Set the room’s formality",
        body: "Use the five-stop Formality slider to choose Free-for-all, Heated, Plainspoken, Structured, or Parliamentary. Parliamentary is the backward-compatible default. It freezes with the proceeding and guides the register and pressure of synthesis, speeches, rulings, ballots, and conclusions, but never replaces a bot’s own diction or identity. Each preset chooses its matching formality; changing it makes the setup Custom and clears advocacy consent.",
        clickLabel: "the Formality slider",
        targetSelector: '[data-tutorial-target="debate-formality"]',
      },
      {
        heading: "Choose the rules of the room",
        body: "Forum is the backward-compatible default and uses opening arguments, challenges, rebuttals, and a verdict. Turnabout is a different format, not a theme: advocates give pressable claims, the Judge or Participant can Press, Object, and Present Evidence, and the neutral moderator decides immediately. At Parliamentary formality those formats speak as the Assembly Chamber and Court of Record; lower levels use direct, ordinary language instead. Every evidence challenge remains bound to one statement and one evidence item frozen before Start. Spectators receive a moderator-led examination. Flyting and Cypher are visible as coming-later productions but cannot be selected or sent to the server. Changing format or formality clears advocacy consent so no bot’s accepted role is silently broadened.",
      ...
      },
      {
        ...
        body: "Open Cast, choose Moderator, For, or Against, then pick from the shared color-grid Library—or use Random actors to roll all three unique seats from your full Library at once. Only duplicate seating is blocked: Powers never make a bot ineligible for a role. A hard-muted moderator contributes visible canonical silence. A moderator nobody in the cast can see or hear still completes a durable hidden turn, but the chamber shows an empty, silent podium and a neutral public ledger moves required procedure forward; each advocate reacts only to that absence. A matching observer such as Light alone receives Ryuk’s words, while a Microscopic moderator remains unseen but audible to everyone. Choose whether you will Judge, participate on one side, or watch.",
        clickLabel: "Cast",
        targetSelector: '[data-tutorial-target="debate-cast"]',
      },
      {
        heading: "Choose whether the room has a Jury",
        body: "The separate Jury switch freezes five unique Library bots at Start, excluding the moderator and advocates, and fills missing seats with stable session-only PRISM archetypes. Saved personas and applicable Powers shape each juror’s running reactions and one private vote; relationship memories and Coffee social history never enter the proceeding. Spectators watch and accept the majority. Judges hear named advice before ruling. Participants never receive deliberation, reactions, voices, individual votes, or reasons—only the winning side and anonymous split.",
        clickLabel: "the five-seat Jury switch",
        targetSelector: '[data-tutorial-target="debate-jury"]',
      },
      {
        heading: "Secure advocacy consent",
        body: "Run the private advocacy check after the exact motion, cast, format, and formality are set. A genuine boundary can decline and cannot be overridden; a willing Devil’s Advocate keeps their identity and receives visible framing. Any later motion, cast, format, or formality change clears the old check.",
...
function currentSignalPowerTutorialBody(step: ModeTutorialStep): string {
  return currentInterruptionRetortTutorialBody(step.body)
    ...
    .replace(
      "The real scene ambience and show-scoped room mix stay live there. Use the Host and Guest voice sliders to balance the cast; Signal remembers each bot’s level for this show. Test voices runs a random two-line soundcheck through configured voices except fully muted cast members, and never creates an episode or transcript. Signal saves that alignment for every episode of the show and dashboard, and close-up pans center on those saved bot positions.",
      "The real scene ambience and show-scoped room mix stay live there. Generated studios also expose separate Host and Guest floor glows: drag vertically to place them and horizontally to resize them. Their extracted microphone masks take each cast member’s accent color without adding a separate foreground layer. Screen treatment controls the saved film grain, while the Lighting lab saves separate Light and Dark underglow opacity and Screen or Overlay blending; new shows begin at 100% Overlay. Use the Host and Guest voice sliders to balance the cast; Signal remembers each bot’s level for this show. Test voices runs a random two-line soundcheck through configured voices except fully muted cast members, and never creates an episode or transcript. Signal saves that alignment and cosmetic treatment for every episode, faithful replay, and the show dashboard, and close-up pans center on those saved bot positions.",
    );
}
