  ...
  chat: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Choose LOCAL or ONLINE as the hard network privacy lane. Private chat is separate: enable it from the new-chat hero when you do not want the conversation or memories saved. Once that conversation begins, a locked Private chat badge remains in the navbar as status, not a switch. Switching to another applet disarms Private chat, so returning Home never starts privately by surprise. Avatar Studio Voice uses clear Accent, Feel, and Voice stages: first place the required Accent pin, then shape Feel, then choose a named Voice. The pin owns pronunciation, so changing from Pia to Rowan—or any other named voice—does not move the bot across regions. Zen player voice settings keep the full-width map in Accent. Click or drag anywhere in the world: the pin stays exactly where you leave it while Prism approximates the nearest broadly regional pronunciation—including New York and Southern U.S.—then set Light, Balanced, or Strong. All accents opens the exact list. Voice range filters the named catalog without exposing engine regions. Original and With accent let you compare the result; dialogue text, memory, and exports stay unchanged.",
        clickLabel: "the LOCAL / ONLINE control",
        targetSelector: '[data-tutorial-target="auto-response-mode"]',
      },
    ],
