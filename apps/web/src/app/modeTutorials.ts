...
WHODUNNIT_TUTORIAL_STEP_SOURCE.body = WHODUNNIT_TUTORIAL_STEP_SOURCE.body
  .replace(
    ...
    "Choose Whodunnit? in Debate format and stay inside Debate Studio's existing Court and Cast rails. On Court, choose Participant to prosecute your filed case or Spectator to watch both counsel bots argue it; Participant keeps Coach, Standard, and Immersive feedback choices, while Judge remains visible but unavailable because PRISM must preside from the sealed Case Bible. Set the shared five-stop court formality and choose whether the standard Debate Jury deliberates. On Cast, use Debate's Library selector, search, and groups to seat four to eight suspects, a prosecutor partner, and a rival defense bot; Surprise me and Wield Prism keep the cast editable before you compile a fictional, non-canonical Case Seed.",
  )
  .replace(
  ...
  .replace(
    ...
    "The Theory Board is point-and-click only: select the culprit, accomplice, method, motive, opportunity, evidence, and testimony from the discovered public record. Private desk notes are view-only there; co-counsel consultation is the sole text-entry exception. Filing freezes those selections and the discovered public case record, then visibly gathers the gallery while Debate prepares a real Turnabout on the existing court stage. Only testimony actually obtained through a submitted scene interview enters the court packet; merely meeting a suspect or filing charges cannot manufacture a quote. The Defense enters the defendant’s courtroom denial, and it stands unless the Participant—or the prosecutor partner in Spectator mode—answers it with admissible physical evidence. The sealed Case Bible never enters the public session, bot prompts, replay, or client. Courtroom gallery ambience and the opening gavel follow the same global Audio toggle and volume as Debate. Use the Turnabout command deck to Press, Present Evidence, or Pass. Filing is irreversible: after court begins, the mansion stays permanently sealed, and three failed evidence challenges end the case immediately. Archive and Resume use the same Turnabout proceeding and gallery record as every other Debate.",
  );
...
const WHODUNNIT_TUTORIAL_STEP: ModeTutorialStep = {
  ...
  body: WHODUNNIT_TUTORIAL_STEP_SOURCE.body
    ...
    .replace(
      ...
      "Type @ in the interview composer or partner composer to mention discovered evidence, committed testimony, a colored suspect name, or the victim. Each suspect interview is private: the current suspect receives only their own prior answers, and another suspect’s statement enters the conversation only when you deliberately @ that testimony in your question. The partner composer can also @ a lead at its current revision, so co-counsel reasons from the exact public thread you are viewing.",
    ),
};
