  ...
  zen: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "AUTO keeps the current response model as Primary, then quietly tries your ordered chain of one to five saved local or online fallbacks if a reply fails validation. Image generation keeps its own LOCAL/ONLINE choice in Images. Voices default to five included PRISM voices; operating-system voices are optional in Voice Settings. Every bot voice uses the local playback effect selected in Avatar Studio—Chorus by default—across PRISM, operating-system, ElevenLabs, Babble, and Bottish playback. In bot customization, choose an ElevenLabs voice from the list or open “Use an exact Voice ID” for a portable override; Prism uses it only for eligible ONLINE speech. The Tone tab gives each bot a Voice Character pad: move left or right to balance low-end weight against high-end clarity, and up or down to trim that bot relative to your account Voice Volume. Voice Settings can narrow those bot menus to one ElevenLabs voice collection. When an eligible bot has a non-neutral mood, Eleven v3 automatically carries that feeling into the next spoken line; neutral speech stays untagged. With Voice Effects on, longer spoken replies may take a quiet mic-ready breath before the line.",
  ...
  botcast: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "Signal keeps one primary speaker on mic at a time: their face comes alive and the transcript follows only the words they have finished saying. The listening host or guest may add a low-key nod, expression, or brief conversational acknowledgement during the line; calm backchannels overlap naturally without creating a turn or entering the transcript. Once the guest is genuinely annoyed, they can try to cut across the host with a brief audible interjection while the host keeps primary transcript ownership. An interruptive host Power can also seize a bounded live opening in a bot guest’s answer: frequency, strength, target, and cooldown come from the reusable Power contract, while human Producer speech, warnings, departures, wraps, closings, and hard speech restrictions stay protected. Signal’s separate immersive reactions still belong to the performing bot, float above that bot, and appear between asterisks in the transcript without becoming fallback dialogue. In a normal bot-guest episode, ask about a detail, refocus, press harder, move on, or lighten up at any time; every cue is private to the host, and the guest only hears what the host says on mic. Producer-guest episodes replace this entire cue-card control room with the bottom answer composer, so the AI host keeps sole editorial control. When a cue arrives early in the host’s own line, they are likely to break off and redirect on mic with an in-character self-correction, even if the live pivot lands a little awkwardly. Once most of the point is already out, the cue stays queued for the host’s next turn. If the guest has the mic or is next, Interrupt guest now plays one of that host’s saved short interjections immediately while the host’s continuation generates. Any unheard remainder of the guest’s line is discarded from the live transcript and replay, so only what reached the audience remains. Wrap it up privately asks the host to steer the exchange to a real ending. The on-air clock shows elapsed episode time and freezes on the final duration. While Signal is on air, app switching and session-changing navbar tools stay locked; Voice remains available for the next line, while read-only Usage and Memories plus Theme remain available. Cut show stops the current line, catches the host slightly off guard, and gives them one quick, tactful sign-off before Signal archives the recording and restores the full chrome; it does not discard the transcript. Choosing any show in the left rail while on air makes the same producer cut before Signal changes shows. Natural endings and producer cuts both land on a short, locally synthesized outro whose end card waits for you to return to the show. A clear in-character guest goodbye ends their turns, preserves the empty-chair aftermath, and gives the host one closing beat. Freeform producer pressure or Press harder can instead earn resistance, a warning, and, rarely, a walkout.",
