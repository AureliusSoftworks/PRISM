  ...
  avatar: {
    ...
    steps: [
      ...
      },
      {
        ...
        body: "The established Eyes and Mouth controls operate the live bot. The display stays fixed while you customize; only Ink Display unlocks drag-to-align and scroll-to-resize. Five floating orbs preview Idle, Blink, Thinking, Sip, and Corporality, with a label on hover. Voice is a required three-stage casting flow—1 Accent, 2 Feel, 3 Voice. Place the Accent pin first; nearby-choice buttons disambiguate dense regions such as Britain, Ireland, Scotland, France, and Germany. Feel shapes character and Pitch, Pace, and Lilt. Voice then offers distinct names such as Pia, Rowan, Iris, Sol, and Mira instead of engine regions, so changing timbre never moves the accent pin. Use the compact audition dock beneath the bot to edit one sample and compare English, Premium, Babble, or Bottish; no preview enters chat. A meaningful customization brings that module's light online. For a surprise, Wield Prism onto the exact control you want to refract instead of hunting through separate randomizer buttons.",
