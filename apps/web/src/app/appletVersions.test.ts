...
describe("applet version helpers", () => {
  ...
  it("tracks the current visual applet versions for release provenance", () => {
    assert.equal(PRISM_APPLETS.chat.version, "1.1");
    assert.equal(PRISM_APPLETS.zen.version, "1.0");
    assert.equal(PRISM_APPLETS.coffee.version, "1.2");
    assert.equal(prismAppletVersionLabel("chat"), "v1.1");
    assert.equal(prismAppletVersionLabel("zen"), "v1.0");
    assert.equal(prismAppletVersionLabel("coffee"), "v1.2");
  });
});
