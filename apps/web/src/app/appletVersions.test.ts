...
describe("applet version helpers", () => {
  ...
  it("tracks the current visual applet versions for release provenance", () => {
    ...
    assert.equal(PRISM_APPLETS.coffee.version, "1.6");
    assert.equal(PRISM_APPLETS.botcast.version, "0.6");
    ...
    assert.equal(prismAppletVersionLabel("coffee"), "v1.6");
    assert.equal(prismAppletVersionLabel("botcast"), "v0.6");
