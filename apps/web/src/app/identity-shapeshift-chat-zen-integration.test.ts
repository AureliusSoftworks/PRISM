...
describe("Shapeshifter Chat/Zen presentation", () => {
  it("reads sticky identityShapeshift for Chat/Zen form with holder voice and target Accent Map", () => {
    ...
    assert.match(
      pageSource,
      /color:\s*state\.targetColor[\s\S]*glyph:[\s\S]*state\.targetGlyph[\s\S]*authored_audio_voice_profile:\s*resolveBotIdentityShapeshiftVoiceV1\([\s\S]*voicePreset:\s*state\.targetVoicePreset[\s\S]*frameMaterialSeed:[\s\S]*state\.targetFrameMaterialSeed/u,
    );
    assert.match(
      pageSource,
