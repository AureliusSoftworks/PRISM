...
describe("corporality-action-sfx resolver", () => {
  it("prefers pack clips over corporality stock", () => {
    ...
    assert.ok(resolved);
    assert.equal(resolved!.source, "pack");
    assert.equal(resolved!.urls[0], "/api/action-sfx-pack/clip?kind=fart");
    assert.equal(resolved!.variantIndex, 2);
  });
  ...
  it("returns null for non-bodily pack kinds without a pack source", () => {
    assert.equal(
      resolveBodilyActionSfxPlayback({
        kind: "laugh",
        corporality: 0.5,
