...
describe("Debate experience", () => {
  ...
  it("animates the live room out, opens completed replays empty, and lifts Debate avatar ink", () => {
    ...
    assert.match(source, /DEBATE_OPENING_CHAMBER_FADE_MS/u);
    ...
    assert.match(source, /data-debate-opening-fade/u);
    assert.match(css, /debate-opening-chamber-fade/u);
  ...
  it("keeps ambience while pressure-mixing the audience in every player role", () => {
    ...
    assert.match(
      source,
      /backgroundUrl=\{\s*ambientAudioActive \? DEBATE_AUDIENCE_MURMUR_URL : null/u,
    );
    assert.match(
      source,
      /grainUrl=\{[\s\S]{0,120}audiencePressureBandTrue[\s\S]{0,80}DEBATE_AUDIENCE_CROSSTALK_URL/u,
    );
    assert.match(
      source,
  ...
  it("bookends a new live Forum and its completed verdict with the Living Chamber ident", () => {
    ...
    assert.match(
      source,
      /setActiveSession\(reuseDebateSessionEventPrefix\(previous, next\)\)[\s\S]{0,260}options\.playIntro[\s\S]{0,280}judgeGavelSmashRef\.current\?\.\("order"\)[\s\S]{0,120}setDebateOpeningGalleryHushed\(true\)[\s\S]{0,420}await playDebateIdent\("intro"\)[\s\S]{0,420}DEBATE_OPENING_CHAMBER_FADE_MS[\s\S]{0,360}setDebateOpeningGalleryHushed\(false\)[\s\S]{0,120}await consumeNewEvents/u,
    );
    assert.match(
      source,
  ...
  it("keeps Jury camera entry and exit automatic for every eligible role", () => {
    ...
    assert.match(
      source,
      /function debateCameraModeForSession[\s\S]{0,220}session\.playerRole === "judge"\) return "auto";[\s\S]{0,120}cameraMode === "jury" \? "auto" : cameraMode/u,
    );
    assert.match(
      source,
    ...
    assert.match(
      source,
      /session\.playerRole === "judge"\s*\? camera\.id === "auto"\s*:\s*camera\.id !== "jury"/u,
    );
  ...
  it("integrates a persona-tinted moderator-camera gavel with restrained procedural Foley", () => {
    ...
    assert.match(
      source,
      /backgroundUrl=\{\s*ambientAudioActive\s*\? DEBATE_AUDIENCE_MURMUR_URL : null/u,
    );
    assert.match(
      source,
