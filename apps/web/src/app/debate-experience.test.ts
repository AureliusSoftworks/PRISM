...
describe("Debate experience", () => {
  ...
  it("restores archived setup without carrying frozen runtime or stale consent", () => {
    ...
    assert.deepEqual(draft.preferredJurorBotIds, [
      null,
      null,
      null,
      null,
  ...
  it("animates the live room out, opens completed replays empty, and lifts Debate avatar ink", () => {
    ...
    assert.match(source, /holdScope="full"/u);
  ...
  it("mounts the live chamber during Spectator pre-bake instead of a fullscreen loader", () => {
    ...
    assert.match(source, /!galleryArriving/u);
    assert.match(css, /data-debate-room-presence="arriving"[\s\S]*\.stageSupport/u);
  });
  ...
  it("keeps manual research real while routing optional synthesis through Wield Prism", () => {
    ...
    assert.match(
      source,
      /\/api\/debates\/exhibits\/synthesize\/cancel-all/u,
    );
    assert.match(
      source,
    ...
    assert.match(source, /The text record is evidence|Tap the picture to pick an emoji/u);
  ...
  it("soft-synthesizes only emoji-only exhibits on Save and Start", () => {
    ...
    assert.match(
      saveSource,
      /custom assets remain unchanged/u,
    );
    assert.match(
      source,
  ...
  it("hands soft exhibit delivery to the global server-owned queue", () => {
    ...
    assert.match(
      backgroundSource,
      /\/api\/soft-asset-jobs\/debate-exhibits/u,
    );
    ...
    const archiveStart = source.indexOf(
      "const synthesizeArchiveExhibitImage",
    );
    ...
    assert.doesNotMatch(
      archiveSource,
      /\/api\/debates\/exhibits\/synthesize/u,
    );
  ...
  it("keeps setup in one studio console with free navigation and one launch action", () => {
    ...
    assert.match(source, /debateJuryDeliberationMouthShape\(/u);
    assert.match(source, /juryDeliberationMouthTickMs/u);
    assert.match(
      source,
      /!silentDeliberationPreparing &&\s*talking &&\s*activeEvent/u,
    );
    assert.match(
      source,
      /thinking:\s*!silentDeliberationPreparing &&\s*thinkingBotId === juror\.id/u,
    );
  ...
  it("lets archived exhibits upload, reuse, and edit their fallback emoji in either theme", () => {
    ...
    assert.match(archiveAssetsSource, /accept="image\/png,image\/jpeg,image\/webp"/u);
    ...
    assert.match(source, /exhibits\/\$\{encodeURIComponent\(exhibit\.id\)\}\/emoji/u);
    assert.match(
      css,
  ...
  it("unifies audience order and semantic intervention behind one contextual gavel", () => {
    ...
    assert.match(
      source,
      /Speech ticks often skip React liveReveal/u,
    );
    ...
    assert.match(source, /spectatorWatchPresentationCompleteRef/u);
    assert.match(
      source,
      /!spectatorWatchPresentationCompleteRef\.current/u,
    );
    assert.match(
      source,
  ...
  it("persists and replays audience order without cancelling the live floor", () => {
    ...
    assert.match(
      source,
      /debate-audience-laugh-into-order:/u,
    );
    assert.match(
      source,
  ...
  it("keeps the full audience look while stabilizing low-cost portrait effects", () => {
    ...
    assert.match(
      page,
      /const galleryAvatarDetails =[\s\S]{0,180}const galleryHasAvatarArt = avatarDetailsHasVisuals\(\s*galleryAvatarDetails,?\s*\);[\s\S]{0,1400}<AvatarDetailsMask[\s\S]{0,260}detailLevel="audience"[\s\S]{0,260}staticRaster[\s\S]{0,140}coreColor="ink"/u,
    );
    assert.match(page, /avatarDetailsColor=\{debateAvatarDetailsColor\}/u);
    assert.match(
      page,
      /staticRaster=\{detailLevel === "audience"\}[\s\S]{0,100}coreColor=\{avatarDetailsCoreColor\}/u,
    );
    assert.match(
      source,
  ...
  it("uses a persistent four-seat Jury camera with a moderator final ballot", () => {
    ...
    assert.match(
      source,
      /const cameraView = galleryArriving\s*\?\s*"wide"/u,
    );
  ...
  it("queues the latest juror thought and keeps Jury comments out of Proceedings", () => {
    ...
    assert.match(
      source,
      /transcriptHeaderActions[\s\S]{0,500}debate-copy-all-review-data[\s\S]{0,900}debate-copy-jury-transcript[\s\S]{0,500}debate-copy-transcript/u,
    );
    assert.match(source, /liveRailPanel === "proceedings"/u);
    assert.match(
      source,
    ...
    assert.match(
      source,
      /const juryRecordReady = sealedCompleted && juryWasSeated/u,
    );
    assert.match(
      source,
      /Keep the live Jury widget through aftermath; swap only after the verdict seals/u,
    );
    assert.doesNotMatch(
      source,
  ...
  it("keeps the evidence drawer close control above app chrome", () => {
    ...
    assert.match(
      source,
      /current === source\.id \? null : source\.id/u,
    );
  });
  ...
  it("keeps ambience while pressure-mixing the audience in every player role", () => {
    ...
    assert.match(
      source,
      /backgroundUrl=\{liveGalleryBackgroundUrl\}/u,
    );
    assert.match(
      source,
    ...
    assert.match(
      source,
      /debateAudienceOrderCallMix\(session\.formality\)/u,
    );
    assert.match(
      source,
  ...
  it("renders response cues as compact chronological Foley notation", () => {
    ...
    const notationStart = source.indexOf(
  ...
  it("offers manual Jury camera once the chamber is open, disabled before then", () => {
    assert.match(
      source,
    ...
    assert.match(
      source,
      /session\.playerRole === "judge"[\s\S]{0,220}camera\.id === "auto"[\s\S]{0,180}camera\.id === "jury"/u,
    );
    assert.match(
      source,
      /return camera\.id !== "jury" \|\| session\.jury\.enabled/u,
    );
    assert.match(
      source,
      /const juryCameraClosed =\s*camera\.id === "jury" &&\s*!debateJuryManualCameraAvailable\(session\)/u,
    );
    assert.match(
      source,
  ...
  it("integrates a persona-tinted moderator-camera gavel with restrained procedural Foley", () => {
    ...
    assert.match(
      source,
      /Ceremonial resume keeps the recess UI/u,
    );
    assert.match(
      source,
    ...
    assert.match(
      source,
      /gavelArmed: activeGavelCue !== null/u,
    );
    assert.match(
      source,
