...
const LEADING_UNMARKED_STAGE_SPOKEN_OPENER_RE =
  /\s+(?=(?:["“'‘])?(?:[A-Z][\p{L}'-]{1,24},|(?:I|I'm|I've|I'd|You|You're|You've|We|We're|A|That's|The|This|These|Those|Each|Every|But|Still|Anyway|Honestly|Honest\s+(?:and|but|or|is|are|isn't|aren't)|Listen|Look,|Well|Ah|Oh|Ar|Arr|Aye|Of|No offense|No,|Yes,|Okay|Sure|Mine|Yours|Consider|Imagine|Suppose|Notice|Think|Let's|Let us|Picture|Now|Yeah|Yup|Hey|Huh|Hmm|Actually|Me|True|False|Maybe|Perhaps|My|So|And|Because|If|When|Where|Why|How|What|Dodging)\b))/gu;

function normalizeUnmarkedStageAction(raw: string): string {
  return raw.replace(/[,.!?;:\s]+$/u, "").trim();
}

const LEADING_UNMARKED_STAGE_SPOKEN_PREFIX_RE =
  /^(?:I|I'm|I've|I'd|Me|My|Mine|You|You're|You've|Your|Yours|We|We're|We've|Our|Ours|If|When|Where|Why|How|What|Who|Which|This|That|These|Those|The|A|An|Every|Each|But|Still|Anyway|Honestly|Listen|Look,|Well|Ah|Oh|No|Yes|Okay|Sure|Maybe|Perhaps|So|And|Because|Now|Yeah|Yup|Hey|Huh|Hmm|Actually|True|False)\b/iu;

function startsLikeSpokenUnmarkedProse(text: string): boolean {
  return LEADING_UNMARKED_STAGE_SPOKEN_PREFIX_RE.test(text.trimStart());
}
...
function looksLikeStandaloneUnmarkedStageAction(text: string): boolean {
  ...
  return (
    ...
    trimmed.length <= 180 &&
    !/[?"]/u.test(trimmed) &&
    !startsLikeSpokenUnmarkedProse(trimmed) &&
...
function extractLeadingUnmarkedStageDirection(text: string): {
  ...
  if (!prefix && startsLikeSpokenUnmarkedProse(rest)) return null;
  ...
  for (const match of rest.matchAll(LEADING_UNMARKED_STAGE_SPOKEN_OPENER_RE)) {
    ...
    if (splitIndex < 4 || splitIndex > 220) continue;
