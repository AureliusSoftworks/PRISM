...
describe("mode tutorials", () => {
  ...
  it("teaches pot-only Coffee without retired service or a player mug", () => {
      const copy = MODE_TUTORIALS.coffee.steps.map((step) => `${step.heading} ${step.body}`).join(" ");
      ...
      assert.doesNotMatch(copy, /Have something made|Make the rounds|I’ll take the…|Surprise me|Standard house blend/u);
      assert.equal(
        MODE_TUTORIALS.coffee.steps.some(
          (step) => step.targetSelector === '[data-tutorial-target="coffee-bar-ritual"]',
        ),
        false,
      );
    });
