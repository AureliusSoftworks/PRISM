...
describe("mode tutorials", () => {
  ...
  it("covers the complete Whodunnit investigation and trial loop", () => {
    ...
    assert.match(step.body, /Recipe Seed/u);
    assert.match(step.body, /Surprise me/u);
    ...
    assert.match(step.body, /reopening starts a fresh pass whose first committed area costs one action/u);
    assert.match(step.body, /every Case Kit application cost one action/u);
    ...
    assert.match(step.body, /Each suspect interview is private/u);
    assert.match(step.body, /only when you deliberately @ that testimony/u);
    ...
    assert.match(step.body, /first committed frozen area in each search pass costs one action/u);
    assert.match(step.body, /every remaining area in that pass is free/u);
    ...
    assert.match(step.body, /Drag a Case Kit tool onto the appropriate inventory item or stage padlock/u);
    assert.match(step.body, /harder cases may contain convincing keys with no matching lock/iu);
    assert.match(step.body, /compact Case Kit during a room search/u);
    assert.match(step.body, /portable locked container puts the whole object in Case inventory/u);
    assert.match(step.body, /never leaves a duplicate padlock on the room stage/u);
    assert.match(step.body, /Stationary locks such as concealed safes remain in the room/u);
    assert.match(step.body, /All visible areas inspected/u);
    assert.match(step.body, /Investigator’s Desk is closed by default/u);
    assert.match(step.body, /arrange discovered records freely/u);
    assert.match(step.body, /drag placed items to move them again/u);
    ...
    assert.match(step.body, /active prosecution counsel/u);
    assert.match(step.body, /passive co-counsel/u);
    assert.match(step.body, /active defense counsel/u);
    assert.match(step.body, /passive client/u);
    assert.match(step.body, /labels, not extra seats or controls/u);
    assert.match(step.body, /submitted scene interview enters the ordered witness chain/u);
    assert.match(step.body, /defendant’s denial remains admissible even without an interview/u);
    assert.match(step.body, /pauses without a twitch timer/u);
    assert.match(step.body, /Press for a source-bound clarification at no cost/u);
    assert.match(step.body, /exact sealed-case pair/u);
    assert.match(step.body, /sealed Case Bible never enters the public session, bot prompts, replay, Archive, or client/u);
    assert.match(step.body, /Archive and Resume use the same Turnabout proceeding/u);
    assert.match(step.body, /Previous or Next/u);
    assert.match(step.body, /Present a frozen evidence or testimony item/u);
    assert.match(step.body, /or Pass/u);
    assert.match(step.body, /Filing is irreversible/u);
    assert.match(step.body, /mansion stays permanently sealed/u);
    assert.match(step.body, /exhausting credibility ends the case immediately/u);
