...
test("Signal keeps the shared authored renderer on its live stage", () => {
  ...
  assert.match(
    pageSource,
    /const signalMannequinProps:[\s\S]*?detailLevel:\s*"full"[\s\S]{0,320}minimumRenderedSizeTier:\s*"full"[\s\S]{0,800}pixelRasterizationEnabled:\s*true[\s\S]{0,160}runtimeEffectsEnabled:\s*!signalLivePerformanceAvatar/u,
  );
