...
test("uses Option as the global Prism Wield modifier", () => {
...
test("uses Option-only Wield on every platform", () => {
  const option = {
    key: "Alt",
    code: "AltLeft",
    altKey: true,
  ...
  assert.equal(isPrismCompanionModifierKey(option, "MacIntel"), true);
  assert.equal(isPrismCompanionModifierKey(option, "Win32"), true);
  assert.equal(
    isPrismCompanionModifierKey(
...
test("does not Wield from Command or Control", () => {
  const base = {
    key: "Meta",
    altKey: false,
    ctrlKey: false,
  ...
  assert.equal(isPrismCompanionModifierKey(base, "MacIntel"), false);
  assert.equal(
    isPrismCompanionModifierKey(
