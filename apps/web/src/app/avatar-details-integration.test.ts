...
describe("Avatar Details Studio integration", () => {
  ...
  it("guards Studio saves and waits for applied Details state before persisting", () => {
    ...
    assert.match(pageSource, /setPendingDetailsSaveKey\(avatarDetailsKey\(avatarDetailsPreview\)\)/);
    assert.match(pageSource, /avatarDetailsKey\(avatarDetails\) !== pendingDetailsSaveKey/);
